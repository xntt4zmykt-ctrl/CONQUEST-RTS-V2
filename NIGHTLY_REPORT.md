# Nightly report — passe 75 (revue PR #188)

**Branche revue :** `cursor/analyse-nocturne-du-codebase-d645` (PR #188, `d65cb63`)  
**Branche de correctifs :** `cursor/analyse-nocturne-du-codebase-29b4`  
**Date :** 2026-08-26  
**Banc :** `./tests/run.sh` — serveur **vert**, client **34/34 vert**. `error()` si un invariant casse (Luau CLI sans `os.exit`).

Revue de PR #188 (`UnitModels` `addWake` `fromEulerAnglesYXZ` — HEAD visuel, V92). Correctifs sûrs, sans merger feel `c62f`/`95ed`/`4885`/`a8e1`/`c299`/`595e`/`bfcc` ni hardening `41e2`/`93f6`.
...
### ISSUE-V94 — UnitModels `LandingRamp` `CFrame.Angles`

**Problème.** V93 ferme `CFrame.Angles` sur les **deux proues** `Bow` (yaw Y=π fixe). Reste la **rampe de débarquement** dans `UnitModels.createBoat` kind==1 :

```
CFrame.new(0, 0.72, -4.45) * CFrame.Angles(math.rad(-8), 0, 0)
```

Pitch X **fixe** -8° (un site, kind==1 amphibie seulement). `Name = LandingRamp`. `WedgePart`. Distinct de V93 (Bow yaw Y=π, carrier + transport/cargo — ne **pas** retoucher les Bow). Distinct de leftover V95 (`MissileBody` yaw Y=90°, cylindre). Distinct de V92 (Wake yaw Y=±11°, `addWake`). Construction de modèle, **pas** 60 Hz interpolation. `addPart` pose `part.CFrame = offset` ; Overlay `place` compose `frame * offset` ensuite. Size `3.25, 0.28, 2.9` / NAVY_DARK puis `Color = faction` / DiamondPlate **inchangés**. Call site `createBoat` kind==1 **seulement** (cargo kind==2 et carrier kind==3 n'ont pas de rampe).

**20K CCU.** 8 clients × pose transport amphibie. Pas d'autorité. Même classe que feel N145 (Angles → `fromEulerAnglesYXZ` inline, pitch X=-8° — **ne pas** cuire un rot, un seul site, même discipline que V93 `bowRot` interdit).

**Faire.** Remplacer `CFrame.Angles(math.rad(-8), 0, 0)` par `CFrame.fromEulerAnglesYXZ(math.rad(-8), 0, 0)` au **seul** site. Hot path : `CFrame.new(0, 0.72, -4.45) * CFrame.fromEulerAnglesYXZ(math.rad(-8), 0, 0)`. Position / Size / Color / Material / Name **inchangés**. Ne **pas** cuire un `rampRot` (recette feel N145 **inline**). Ne **pas** retoucher `Bow` (V93 déjà). Ne **pas** éditer `addWake` (V92 déjà). Ne **pas** éditer `MissileBody` / `Fin`. Ne **pas** changer `UnitModels.place` (V73 déjà). Ne **pas** changer Overlay (V71/V72/V78 déjà). Ne **pas** merger feel `95ed`.

**Contraintes.** Client-only. **V94 visual ≠ V93 (Bow yaw Y=π) ≠ V92 (Wake yaw Y=±11°) ≠ V95 leftover (`MissileBody` Y=90°).** Recette feel N145 — **ne pas merger** `95ed`. Client 34/34 (check navires V93 Bow `# == 1` carrier Y == -0.2 / transport Y == 0.05 **doit rester vert**. Check V92 Wake `# == 2` Y == -0.68 **doit rester vert**. Check V91 Interceptor `# == 4` **doit rester vert**. Check V78 LaunchWake **doit rester vert**). **Ne pas** éditer le serveur. **Ne pas** éditer `BuildingModels.luau`. **Ne pas** éditer `WorldRenderer.luau`. **Ne pas** éditer `Effects.luau`. **Ne pas** éditer `Overlay.luau`. **Ne pas** éditer `Bow` / `addWake` / `MissileBody`.

**Tester.** Check « navires, missiles et interpolation » : après spawn 3 navires, id 1 (kind==1 LandingShip) a 1 Part `Name == LandingRamp`. Offset local `Y == 0.72`. id 2 cargo et id 4 carrier : 0 LandingRamp. Missile skip. V93 Bow `# == 1` **doit rester vert**. V92 Wake `# == 2` **doit rester vert**. V78 LaunchWake Y = OCEAN+0.12 **doit rester vert**. `./tests/run.sh`. Client 34/34.

**Fichiers.** `UnitModels.luau` (`LandingRamp` **seulement** la ligne CFrame). `tests/client.luau` check navires + commentaire leftover. `Bow` **non**. `addWake` **non**. `BuildingModels.luau` **non**. `WorldRenderer.luau` **non**. `Effects.luau` **non**. `Overlay.luau` **non**. `init.client.luau` **non**.

---

## Hors scope volontaire

- Merger feel `c62f`/`95ed`/`4885`/`a8e1`/`c299`/`595e`/`bfcc` / hardening `41e2`/`93f6` sur #188.
...
- Feel N144 `Bow` — **porté ici** (V93 inline, yaw Y=π fixe, **pas** merger `c62f`).
- Feel N145 `LandingRamp` — leftover V94 (pitch X=-8°, kind==1, **pas** merger `95ed`).
...
- UnitModels `LandingRamp` `CFrame.Angles` — leftover V94 (pitch X=-8° fixe, kind==1 amphibie, recette feel N145 inline, **pas** un rot cuit, **pas** merger `95ed`). Proues **non** retouchées.
- `CFrame.Angles` de construction restants (`LandingRamp`, `MissileBody`, ailettes) — une fois, pas 60 Hz interpolation. Prochaine après V93 = UnitModels `LandingRamp` (feel N145, pitch X=-8°).
...
Serveur : invariants + P0 + or plat + `removePlayer` refund + embargo auto + cap 3 transports + passe 16–74 inchangées (passe 75 = client-only).  
...
Client V93 : même check navires — chaque modèle navire a 1 Part `Name == Bow`. Offset local carrier (id 4) `Y == -0.2` ; transport/cargo (id 1/2) `Y == 0.05`. Missile (id 3) 0 Bow. Distinct de V92 Wake Y == -0.68. V92 Wake `# == 2` **doit rester vert**. V78 LaunchWake `# == 3` **doit rester vert**. Leftover V94 `LandingRamp` **non testé ici**.  
