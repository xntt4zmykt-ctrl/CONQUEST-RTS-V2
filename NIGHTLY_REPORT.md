# Nightly report — passe 71 (revue PR #180)

**Branche revue :** `cursor/analyse-nocturne-du-codebase-f9ec` (PR #180, `5a37cdb`)  
**Branche de correctifs :** `cursor/analyse-nocturne-du-codebase-9fd4`  
**Date :** 2026-08-26  
**Banc :** `./tests/run.sh` — serveur **vert**, client **34/34 vert**. `error()` si un invariant casse (Luau CLI sans `os.exit`).

Revue de PR #180 (`BuildingModels` `cylinder()` `fromEulerAnglesYXZ` — HEAD visuel, V88). Correctifs sûrs, sans merger feel `a8e1`/`4885`/`c299`/`595e`/`bfcc` ni hardening `41e2`/`93f6`.
...
- **PR #180 :** V88 intact (`fromEulerAnglesYXZ` Z=90° fixe, helper `cylinder()`, pas de `cylinderRot`). Toits V89 **fermés ici**. Rock V87 / TreeTrunk V86 / BuildRing V85 / OceanGlint V84 / SelectionRing V83 / ConquestPulse V82 / Shockwave V81 / DeliveryPulse V80 / LandingSplash V79 / LaunchWake V78 / early-out V77 / Size V76 / rot V75 **non retouchés**. Rien à revert.

---

## Specs worker (reste)

Ne pas merger feel `a8e1`/`4885`/`c299`/`595e`/`bfcc` ni hardening `41e2`/`93f6` sur cette branche sans rebase. Porter **une** recette à la fois. Feel N129 (footprint hauteur **0.4**, **pas** de pulse) ≠ visual V75/V76/V77 — ne pas merger. Feel N134 `Y = 3` **non** porté (visuel = surface + 0.8). Feel N135 OceanGlint **fermée** (V84) — **ne pas merger** `595e`. Feel N136 BuildRing **fermée** (V85, PR #175) — **ne pas merger** `595e`. Feel N137 TreeTrunk **fermée** (V86, PR #177) — **ne pas merger** `c299`. Feel N138 Rock **fermée** (V87, PR #179) — **ne pas merger** `c299`. Feel N139 `cylinder()` **fermée** (V88, PR #180) — **ne pas merger** `a8e1`. Feel N140 toits usine **fermée** ici (V89 inline) — **ne pas merger** `a8e1`. Feel N141 portes silo — **ne pas merger** `4885`.
...
### ISSUE-V90 — BuildingModels `buildSilo` portes `CFrame.Angles`

**Problème.** V89 ferme `CFrame.Angles` sur les **toits sheds** de `buildFactory` (Z=-12° fixe, `BuildingModels`). Restent les **deux portes** du silo dans `buildSilo` :
...
Z **fixe** ±18° (portes blindées ouvertes, deux sites). `Name = SiloDoor`. Distinct de V89 (toits usine, Z=-12°, **même fichier**, `buildFactory` — ne **pas** retoucher les toits). Distinct de V88 (`cylinder()` helper, Z=90°, **même** `buildSilo` pour le tube/missile — ne **pas** retoucher le helper). Distinct de V87 (Rock yaw `phase` variable, `WorldRenderer`). Distinct de leftover V91 (rampes SAM, pitch X=-31°, `buildSam`). Construction de modèle, **pas** 60 Hz interpolation. `block()` pose d’abord `CFrame.new(±3.6, 1.3, 0)` ; la ligne **écrase** ensuite. STEEL / Size `4.4, 0.45, 6.5` / Material Metal **inchangés**.

**20K CCU.** 8 clients × pose silo (chargement + capture + upgrade palier). Pas d’autorité. Même classe que feel N141 (Angles → `fromEulerAnglesYXZ` inline, Z=±18° — **ne pas** cuire un rot, translations **et** signes distincts, comme V86 `trunkRot` / V88 `cylinderRot` / V89 `roofRot` interdits).

**Faire.** Remplacer `CFrame.Angles(0, 0, math.rad(18))` / `CFrame.Angles(0, 0, math.rad(-18))` par `CFrame.fromEulerAnglesYXZ(0, 0, math.rad(18))` / `CFrame.fromEulerAnglesYXZ(0, 0, math.rad(-18))`. Hot path : `doorA.CFrame = CFrame.new(-3.6, 1.3, 0) * CFrame.fromEulerAnglesYXZ(0, 0, math.rad(18))` et `doorB.CFrame = CFrame.new(3.6, 1.3, 0) * CFrame.fromEulerAnglesYXZ(0, 0, math.rad(-18))`. Position / Size / Color / Material / Name **inchangés**. Ne **pas** cuire un `doorRot` (signes Z opposés + translations distinctes, recette feel N141 **inline**). Ne **pas** retoucher `cylinder()` (V88 déjà). Ne **pas** éditer les toits `buildFactory` (V89 déjà). Ne **pas** éditer le tube / missile / ogive. Ne **pas** toucher les rampes SAM (leftover V91, pitch X=-31°). Ne **pas** changer `BuildingModels.animate` (V60/V70 déjà). Ne **pas** changer `playConstruction` (V85 déjà). Ne **pas** changer `WorldRenderer.luau` (V86/V87 déjà). Ne **pas** merger feel `4885`.

**Contraintes.** Client-only. **V90 visual ≠ V89 (toits Z=-12°) ≠ V88 (`cylinder()` Z=90°) ≠ V87 (Rock yaw `phase`) ≠ V91 leftover (SAM pitch X=-31°).** Recette feel N141 — **ne pas merger** `4885`. Client 34/34 (check modeles V89 toits palier 1 **doit rester vert**. Check V88 Chimney cylindre **doit rester vert**. Check silo StrategicMissile cylindre **doit rester vert**. Check construction V87 Rock WedgePart **doit rester vert**. Check TreeTrunk V86 cylindre **doit rester vert**. Check BuildRing V85 Y == 2 **doit rester vert**). **Ne pas** éditer le serveur. **Ne pas** éditer `WorldRenderer.luau`. **Ne pas** éditer `Effects.luau`. **Ne pas** éditer `Overlay.luau`. **Ne pas** éditer `UnitModels.luau`. **Ne pas** éditer `cylinder()`. **Ne pas** éditer `buildFactory`. **Ne pas** éditer `buildSam`. **Ne pas** éditer `playConstruction`.

**Tester.** Check « modeles procéduraux : le palier change la silhouette » : après `Building.create(SILO, …)`, deux `SiloDoor` présents, `Y == 1.3 * WORLD_SCALE`. V89 toits `# == 2` palier 1 **doit rester vert**. V88 Chimney / StrategicMissile **doit rester vert**. V85 BuildRing Y == 2 **doit rester vert**. `./tests/run.sh`. Client 34/34.

**Fichiers.** `BuildingModels.luau` (`buildSilo` **seulement** les deux lignes CFrame des portes). `tests/client.luau` check modeles + commentaire leftover. `cylinder()` **non**. `buildFactory` **non**. `WorldRenderer.luau` **non**. `Effects.luau` **non**. `Overlay.luau` **non**. `playConstruction` **non**. `buildSam` **non**. `init.client.luau` **non**.

---

## Hors scope volontaire

- Merger feel `a8e1`/`4885`/`c299`/`595e`/`bfcc` / hardening `41e2`/`93f6` sur #180.
...
- Feel N140 toits usine — **porté ici** (V89 inline, Z=-12° fixe, **pas** merger `a8e1`).
...
- BuildingModels `buildSilo` portes `CFrame.Angles` — leftover V90 (Z=±18° fixe, deux sites, recette feel N141 inline, **pas** un rot cuit, **pas** merger `4885`). Toits **non** retouchés.
- `CFrame.Angles` de construction restants (`addWake`, `Bow`, ailettes, rampes SAM) — une fois, pas 60 Hz interpolation. Prochaine après V90 = rampes SAM (feel N142, pitch X=-31°, `buildSam`).
...
Serveur : invariants + P0 + or plat + `removePlayer` refund + embargo auto + cap 3 transports + passe 16–70 inchangées (passe 71 = client-only).  
...
Client V89 : même check modeles — palier 1, 2 toits Size `2.3, 0.35, 7` × `WORLD_SCALE`, `Y == 4.9 * WORLD_SCALE`. V88 Chimney / StrategicMissile **doit rester vert**. V85 BuildRing Y == 2 **doit rester vert**. Leftover V90 portes silo **non testé ici**.  
