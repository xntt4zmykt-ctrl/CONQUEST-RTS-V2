# Nightly report — passe 55 (revue PR #146)

**Branche revue :** `cursor/analyse-nocturne-du-codebase-9ab9` (PR #146, `ffcf95d`)  
**Branche de correctifs :** `cursor/analyse-nocturne-du-codebase-9793`  
**Date :** 2026-08-26  
**Banc :** `./tests/run.sh` — serveur **vert**, client **34/34 vert**. `error()` si un invariant casse (Luau CLI sans `os.exit`).

Revue de PR #146 (`Overlay.stepInterpolation` roulis navire — HEAD visuel, V71). Correctifs sûrs, sans merger feel `4d8e`/`b19e`/`bec6` ni hardening `4c70`/`24a7`.
...
- **PR #146 :** roulis navire `fromEulerAnglesYXZ` (V71) intact. Banc navires leftover V65 / V55 / V52 + premier `stepInterpolation` immobile sans compose. Radar/Flag/Boom V70 intact. Camion `segRot` (V64) intact. Camera V69 intact. Rien à revert.

---

## Specs worker (reste)

Ne pas merger feel `4d8e`/`b19e`/`bec6`/`ab04` ni hardening `4c70`/`24a7` sur cette branche sans rebase. Porter **une** recette à la fois.
...
### ISSUE-V73 — `UnitModels.place` radar / flag `CFrame.Angles` 60 Hz

**Problème.** V72 ferme le spin des roues camion. Reste, **une fois par frame par pièce radar/flag d’unité** dans `UnitModels.place` (appelé depuis `Overlay.stepInterpolation` pour chaque unité interpolée) :

```
if piece.role == "radar" then
    localFrame = localFrame * CFrame.Angles(0, time * 2.2, 0)
elseif piece.role == "flag" then
    localFrame = localFrame * CFrame.Angles(math.sin(time * 5) * 0.06, 0, 0)
```

`frame` Overlay a **déjà** translation × yaw (V65) ± roulis (V71). `piece.offset` est une pose construction. Donc on compose `frame * (offset * euler)` — **pas** un `RestCFrame` identité BuildingModels, **pas** un `frame * euler` Overlay. Radar : `fromEulerAnglesYXZ(0, time * 2.2, 0)` ≡ `Angles(0, yaw, 0)` (`rx=0`, `rz=0` → Y seul). Flag : `fromEulerAnglesYXZ(sin(time*5)*0.06, 0, 0)` ≡ `Angles(pitch, 0, 0)` (`ry=0`, `rz=0` → X seul). Fréquences 2.2 / 5 et amplitude flag 0.06 **inchangées**. Distinct de V70 (BuildingModels Radar/Flag/Boom, rest identité, noms `"Radar"` / `"CapitalFlag"` / `"PortCraneBoom"`). Distinct de V71 (roulis navire Overlay, deux `sin`, compose sur le `frame` unité). Distinct de V72 (roues camion Overlay, `Name == "Wheel"`, `progress * π * 20`). Distinct des `CFrame.Angles` de **construction** (`addWake`, `Bow`, ailettes missile) — une fois à `create`, pas 60 Hz. Distinct de `piece.role == "flame"` `Size = Vector3.new` (leftover séparé, pas CFrame). Distinct de wake Transparency / trail / light (pas CFrame).

**20K CCU.** Leftover V72. 8 clients × 60 Hz × N unités (porte-avions radar + transports flag) × `CFrame.Angles` + compose. Pas d’autorité (silhouette cosmétique). Un euler faux casserait la rotation du radar (Y × 2.2) ou le battement du drapeau (X × 0.06). Les rôles wake/flame/trail/light n’ont pas ce compose — ne pas l’ajouter.

**Faire.**

1. Dans `UnitModels.place` seulement, branches `role == "radar"` et `role == "flag"` : poser `localFrame = localFrame * CFrame.fromEulerAnglesYXZ(...)` avec les **mêmes trois nombres** qu’avant (`0, time * 2.2, 0` radar ; `sin(time * 5) * 0.06, 0, 0` flag). Plus de `CFrame.Angles` 60 Hz sur le hot path unités. Fréquences / amplitude **inchangées**. Autres rôles (wake Transparency, flame Size, trail, light) **inchangés**.

2. **Garder la rotation.** Ne **pas** convertir en translation (V59 feuillage). Ne **pas** cuire l’euler dans `piece.offset` (le `time` change chaque frame). Ne pas « fermer » Overlay roulis (V71 déjà). Ne pas « fermer » Overlay roues (V72 déjà). Ne pas « fermer » BuildingModels Radar/Flag/Boom (V70 déjà — **autre module**, rest identité).

3. Ne **pas** éditer `Overlay.luau` (V71 / V72 déjà). Ne pas « fermer » les `CFrame.Angles` de construction (`addWake`, `Bow`, missile). Ne pas « fermer » `flame` `Size = Vector3.new`. Ne pas merger feel.

**Contraintes.** Client-only. **V73 visual ≠ V72 (roues camion Overlay) ≠ V71 (roulis navire Overlay) ≠ V70 (BuildingModels rest identité).** Non réentrant (`place` une fois / pièce / frame, un appelant = Overlay). Sans unité interpolée : `place` n’est pas atteint. Client 34/34 (banc « navires, missiles et interpolation » leftover V71 / V65 / V55 / V52 **doit rester vert** — le premier `stepInterpolation` immobile ne compose pas le roulis Overlay ; `UnitModels.place` **tourne quand même** pour radar/flag même immobile. Banc « pose et capture » leftover V72 / V64 / V57 **doit rester vert**. Check modeles V70 **doit rester vert**. Check camera V69 **doit rester vert**). **Ne pas** éditer le serveur.

**Tester.** Banc client navires **doit rester vert**. Check pose/capture V72 **doit rester vert**. Check modeles V70 **doit rester vert**. Check camera V69 **doit rester vert**. `./tests/run.sh`. Client 34/34.

**Fichiers.** `UnitModels.luau` (`place` branches `role == "radar"` et `role == "flag"` **seulement**). `tests/client.luau` **seulement si** le check navires ne mentionne pas encore V73 (commentaire leftover, **garder** V71/V65). `Overlay.luau` **non**. `BuildingModels.luau` **non**. `WorldCamera.luau` **non**.

---

## Hors scope volontaire

- Merger feel `4d8e`/`b19e`/`bec6`/`ab04` / hardening `4c70`/`24a7` sur #146/#143.
...
- `UnitModels.place` radar/flag `CFrame.Angles` = leftover V73 (ne pas éditer UnitModels dans V72 — V72 est fermé, V73 est la prochaine passe).
- `UnitModels.place` flame `Size = Vector3.new` 60 Hz — leftover séparé (ne pas éditer flame dans V73).
- `CFrame.Angles` de construction (`addWake`, `Bow`, ailettes, `WorldRenderer` glint/tronc, `PlacementPreview` footprint) — une fois, pas 60 Hz interpolation.
...
Serveur : invariants + P0 + or plat + `removePlayer` refund + embargo auto + cap 3 transports + passe 16–54 inchangées (passe 55 = client-only).  
...
Client V71 : même check navires — roulis `fromEulerAnglesYXZ(rx, 0, rz)`, plus de `CFrame.Angles` 60 Hz. Premier `stepInterpolation` (unités immobiles) ne compose pas. Leftover V70 modeles **doit rester vert**. Leftover V65 / V55 / V52 **doivent rester verts**. Leftover V64 `segRot` **doit rester vert**. Leftover V69 camera `focusX` **doit rester vert**.  
