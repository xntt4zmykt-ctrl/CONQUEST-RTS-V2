# Nightly report — passe 69 (revue PR #177)

**Branche revue :** `cursor/analyse-nocturne-du-codebase-75ce` (PR #177, `25cbbc2`)  
**Branche de correctifs :** `cursor/analyse-nocturne-du-codebase-2b04`  
**Date :** 2026-08-26  
**Banc :** `./tests/run.sh` — serveur **vert**, client **34/34 vert**. `error()` si un invariant casse (Luau CLI sans `os.exit`).

Revue de PR #177 (`WorldRenderer` TreeTrunk / SavannaTrunk `fromEulerAnglesYXZ` — HEAD visuel, V86). Correctifs sûrs, sans merger feel `c299`/`595e`/`bfcc` ni hardening `41e2`/`93f6`.
...
- **PR #177 :** V86 intact (`fromEulerAnglesYXZ` Z=90° fixe, TreeTrunk cylindre, pas de `trunkRot`). Rock V87 **fermé ici**. OceanGlint V84 / SelectionRing V83 / ConquestPulse V82 / Shockwave V81 / DeliveryPulse V80 / LandingSplash V79 / LaunchWake V78 / early-out V77 / Size V76 / rot V75 **non retouchés**. Rien à revert.

---

## Specs worker (reste)

Ne pas merger feel `c299`/`595e`/`bfcc` ni hardening `41e2`/`93f6` sur cette branche sans rebase. Porter **une** recette à la fois. Feel N129 (footprint hauteur **0.4**, **pas** de pulse) ≠ visual V75/V76/V77 — ne pas merger. Feel N134 `Y = 3` **non** porté (visuel = surface + 0.8). Feel N135 OceanGlint **fermée** (V84) — **ne pas merger** `595e`. Feel N136 BuildRing **fermée** (V85, PR #175) — **ne pas merger** `595e`. Feel N137 TreeTrunk **fermée** (V86, PR #177) — **ne pas merger** `c299`. Feel N138 Rock **fermée** ici (V87 inline) — **ne pas merger** `c299`. Feel N139 `cylinder()` — **ne pas merger** `a8e1`.
...
### ISSUE-V88 — BuildingModels `cylinder()` `CFrame.Angles`

**Problème.** V87 ferme `CFrame.Angles` sur les **rochers** (`Rock`, yaw `phase` variable, `WorldRenderer.buildDecorations`). Reste le helper mesh **`cylinder()`** dans `BuildingModels.luau` :

```
part.CFrame = CFrame.new(offset) * CFrame.Angles(0, 0, math.rad(90))
```

Un helper : Z **fixe** 90° (cylindre Roblox pointe X, on redresse). Call sites `buildFactory` (cheminées + réservoir) et `buildSilo` (tube + missile) — **ne pas** les éditer, seulement le helper. Distinct de V87 (Rock yaw `phase` variable, `WorldRenderer`). Distinct de V86 (TreeTrunk, même Z=90° mais `WorldRenderer.buildDecorations`). Distinct de V85 (BuildRing, `playConstruction`, **pas** le helper `cylinder()`). Distinct de V84 (OceanGlint yaw `angle` variable, `buildOcean`). Construction de modèle, **pas** 60 Hz interpolation. `Shape = Cylinder` **déjà** posé **avant** le CFrame. `block()` pose d’abord `CFrame.new(offset)` ; `cylinder()` **écrase** ensuite.

**20K CCU.** 8 clients × pose usine / silo (chargement + capture). Pas d’autorité. Même classe que feel N139 (Angles → `fromEulerAnglesYXZ` inline, Z=90° fixe — **ne pas** cuire un rot, translations distinctes par `offset`, comme V86 `trunkRot` interdit).

**Faire.** Remplacer `CFrame.Angles(0, 0, math.rad(90))` par `CFrame.fromEulerAnglesYXZ(0, 0, math.rad(90))`. Hot path : `part.CFrame = CFrame.new(offset) * CFrame.fromEulerAnglesYXZ(0, 0, math.rad(90))`. `offset` / `size` / `color` / `Shape = Cylinder` **inchangés**. Ne **pas** cuire un `cylinderRot` (translations distinctes, recette feel N139 **inline** — même raison que V86 `trunkRot`). Ne **pas** éditer les call sites `buildFactory` / `buildSilo`. Ne **pas** toucher les toits usine (leftover V89, Z=-12°). Ne **pas** changer `BuildingModels.animate` (V60/V70 déjà). Ne **pas** changer `playConstruction` (V85 déjà). Ne **pas** changer `WorldRenderer.luau` (V86/V87 déjà). Ne **pas** merger feel `a8e1`.

**Contraintes.** Client-only. **V88 visual ≠ V87 (Rock yaw `phase`) ≠ V86 (TreeTrunk WorldRenderer) ≠ V85 (BuildRing `playConstruction`).** Recette feel N139 — **ne pas merger** `a8e1`. Client 34/34 (check modeles **doit rester vert**. Check construction V87 Rock WedgePart **doit rester vert**. Check TreeTrunk V86 cylindre **doit rester vert**. Check BuildRing V85 Y == 2 **doit rester vert**. Check calques V83 **doit rester vert**. Check vagues V82 **doit rester vert**). **Ne pas** éditer le serveur. **Ne pas** éditer `WorldRenderer.luau`. **Ne pas** éditer `Effects.luau`. **Ne pas** éditer `Overlay.luau`. **Ne pas** éditer `UnitModels.luau`. **Ne pas** éditer `playConstruction`. **Ne pas** éditer les toits `buildFactory`.

**Tester.** Check « modeles procéduraux : le palier change la silhouette » : après `Building.create(FACTORY, …)`, `Chimney` présent ; `Shape == Enum.PartType.Cylinder`. V85 BuildRing Y == 2 **doit rester vert**. V87 Rock WedgePart **doit rester vert**. V86 TreeTrunk cylindre **doit rester vert**. `./tests/run.sh`. Client 34/34.

**Fichiers.** `BuildingModels.luau` (`cylinder()` **seulement**, la ligne CFrame). `tests/client.luau` check modeles + commentaire leftover. `WorldRenderer.luau` **non**. `Effects.luau` **non**. `Overlay.luau` **non**. `playConstruction` **non**. `buildFactory` toits **non**. `buildSilo` call sites **non**. `init.client.luau` **non**.

---

## Hors scope volontaire

- Merger feel `c299`/`595e`/`bfcc`/`a8e1` / hardening `41e2`/`93f6` sur #177.
...
- Feel N138 Rock — **porté ici** (V87 inline, yaw `phase` variable, **pas** merger `c299`).
...
- BuildingModels `cylinder()` `CFrame.Angles` — leftover V88 (Z=90° fixe, helper mesh, recette feel N139 inline, **pas** un rot cuit, **pas** merger `a8e1`). Call sites `buildFactory` / `buildSilo` **non** édités.
- `CFrame.Angles` de construction restants (`addWake`, `Bow`, ailettes, toits usine, portes silo, rampes SAM) — une fois, pas 60 Hz interpolation. Prochaine après V88 = BuildingModels toits usine (feel N140, Z=-12°, `buildFactory`).
...
Serveur : invariants + P0 + or plat + `removePlayer` refund + embargo auto + cap 3 transports + passe 16–68 inchangées (passe 69 = client-only).  
...
Client V87 : même check construction — au moins un Part `Name == Rock` dans `world.decorations` ; `ClassName == WedgePart`. V86 TreeTrunk cylindre **doit rester vert**. Leftover V88 `cylinder()` **non testé ici**.  
