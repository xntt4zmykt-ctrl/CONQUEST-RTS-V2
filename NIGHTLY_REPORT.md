# Nightly report — passe 47 (revue PR #126)

**Branche revue :** `cursor/analyse-nocturne-du-codebase-f5e9` (PR #126, `e52890f`)  
**Branche de correctifs :** `cursor/analyse-nocturne-du-codebase-3062`  
**Date :** 2026-08-26  
**Banc :** `./tests/run.sh` — serveur **vert**, client **34/34 vert**. `error()` si un invariant casse (Luau CLI sans `os.exit`).

Revue de PR #126 (`Overlay.applyRouteProgress` lookAt chantier cuit — HEAD visuel). Correctifs sûrs, sans merger feel `04e7`/`8f41` ni hardening `46a1`.
...
- **PR #126 :** lookAt chantier `segment.rot` (V63) intact. Banc pose/capture → Parts Road stables, `oy` asphalt ≠ shoulder, `rot` à l’origine et `rawequal` après deux frames. Rien à revert.
...
### ISSUE-V65 — Overlay unités lookAt deux Vector3 60 Hz

**Problème.** V64 ferme le lookAt camion. Reste, **par unité interpolée, à chaque frame** : `CFrame.lookAt(Vector3.new(worldX, py, worldZ), Vector3.new(worldX + lookX, py, worldZ + lookZ))`. Deux allocs Vector3. Distinct de V64 (camion `segRot` cuit — direction de segment **constante**) : ici le look **change** chaque frame tant que `mag > 0.01` (suit `targetX`/`targetY`). Immobile = `lookX = 0`, `lookZ = -1` (regard −Z, même pose que `CFrame.new(x,y,z)`). Recette : `yaw = math.atan2(lookX, -lookZ)` ; `frame = CFrame.new(worldX, py, worldZ) * CFrame.fromEulerAnglesYXZ(0, yaw, 0)`. Immobile : `atan2(0, 1) = 0` → identité. Plus de `CFrame.lookAt` 60 Hz unités. `CFrame.Angles` roulis navire **conservé** (appliqué **après** la pose). Extra missile **inchangé**.

**20K CCU.** Leftover V64. 8 clients × 60 Hz × N unités (navires + missiles) × 2 Vector3 lookAt. Pas d’autorité (pose cosmétique). Un yaw faux casserait le cap des flottes et la cloche missile.

**Faire.**

1. Dans la boucle `self.units` de `stepInterpolation` seulement : garder lerp `currentX`/`currentY` (V55) et conversion monde nombres (V56). Remplacer le lookAt deux Vector3 par `CFrame.new(worldX, py, worldZ) * CFrame.fromEulerAnglesYXZ(0, math.atan2(lookX, -lookZ), 0)`. Branche `mag <= 0.01` : `lookX = 0`, `lookZ = -1` déjà → yaw 0. Roulis navire `CFrame.Angles` **après** (inchangé). `UnitModels.place` **inchangé**. Camion `segRot` **inchangé** (V64). Chantier `segment.rot` **inchangé** (V63). Lift / lerp nombres / câble / houle / feuillage **inchangés**. Extra missile **inchangé** (V52). `targetX` **inchangé** (V55).
2. Ne **pas** éditer `BuildingModels.luau` / `WorldRenderer.luau` / `UnitModels.luau` / `HUD.luau` / `WorldSpace.luau` / `WorldCamera.luau`. Ne pas recycler explosion / wake / splash (événement). Ne pas changer la formule `worldX = x * TILE - HALF_W + TILE_HALF`. Après V64.
3. Ne pas porter Overlay camion (V64 déjà) ni chantier (V63 déjà). Ne pas convertir camera `lookAt` (1/frame, leftover V66). Ne pas « fermer » `Size` chantier (API). Ne pas « fermer » `CFrame.Angles` roues camion (spin réel). Ne pas cuire un `segRot` d’unité (le look n’est pas constant).

**Contraintes.** Client-only. **V65 visual ≠ V64 (camion) ≠ V56 (nombres monde, un lookAt restait) ≠ V63 (chantier).** Non réentrant. Client 34/34 (banc « navires, missiles et interpolation » — leftover V56 `currentX`/`currentY` avance **doit rester vert** ; leftover V64 `segRot` `rawequal` **doit rester vert**). **Ne pas** éditer le serveur. Immobile : pose identique à `CFrame.new(worldX, py, worldZ)`. `fromEulerAnglesYXZ(0, atan2(lookX, -lookZ), 0)` : −Z pointe vers `(lookX, 0, lookZ)` (même convention que `lookAt`).

**Tester.** Banc client navires **doit rester vert** : leftover V56 (`currentX` **et** `currentY` avancent après cible déplacée, premier `stepInterpolation` immobile ne casse pas) **et** leftover V55 (`targetX` mute) **et** leftover V52 (`extra` missile). Check pose/capture leftover V64 (`segRot` origine, `rawequal` après deux frames, leftover V63 `rot` chantier, leftover V57 dispatch) **doit rester vert**. `./tests/run.sh`. Client 34/34.

**Fichiers.** `Overlay.luau` (boucle `self.units` de `stepInterpolation` seulement). `tests/client.luau` **seulement si** un assert dans le check « navires, missiles et interpolation ». `WorldCamera.luau` **non**. `UnitModels.luau` **non**. `BuildingModels.luau` **non**.

---

## Hors scope volontaire

- Merger feel `04e7`/`8f41` / hardening `46a1` sur #46/#126.
...
- `CFrame` / `Vector3` unités dans `stepInterpolation` 60 Hz — **fermé** (V56). V55 ne touche que les nombres cible / lerp. `CFrame.Angles` roulis navire **conservé** (sinon `UnitModels.place` perd le tangage). `CFrame.lookAt(Vector3.new, Vector3.new)` unités reste : leftover V65. Camion lookAt — **fermé** (V64).
...
- `applyRouteProgress` lookAt deux Vector3 60 Hz — **fermé** (V63). V62 ne touche que le lerp nombres. `Size` = API, leftover après V63. LookAt camion = **fermé** (V64). LookAt unités = leftover V65.
- Overlay camion lookAt deux Vector3 60 Hz — **fermé** (V64). V63 ne touche que le chantier. Unités lookAt = leftover V65 (look suit `targetX` chaque frame, pas cuisable par segment).
- Overlay unités lookAt deux Vector3 60 Hz — **ouvert** (V65). V64 ne touche que le camion. Camera `lookAt` = leftover V66 (1/frame, pas N unités).
- `UnitModels.place` `Vector3.new` Size flamme missile — leftover séparé (API Size, ne pas éditer UnitModels dans V65). Radar / flag unité `CFrame.Angles` = rotation réelle.
...
Serveur : invariants + P0 + or plat + `removePlayer` refund + embargo auto + cap 3 transports + passe 16–46 inchangées (passe 47 = client-only).  
