...
| Claim #204 | Réalité à l’ouverture |
|---|---|
| Overlay Shockwave free-list (N163) | Oui. `shockFree` pop O(1). `Parent = nil` + push si encore parenté, pas Destroy. Name `Shockwave`. Reset Size `(0.32, 5, 5)` + Transparency `0.12` + Color `255,229,181`. Euler N133 inline. Blast `Destroy` conservé. BlastSmoke `Destroy` **alors**. |
...
**Pourquoi 20K CCU :** leftover N151. 8 clients × ogives × 60 Hz × `Vector3` pour un pulse de Size. Pas d’autorité. BlastSmoke **déjà** N164 — ne pas y revenir. Shockwave **déjà** N163 — ne pas y revenir. Visual V74 freeze **interdit** (feel ≠ visual). **Passes 61–73 ont refusé de livrer N152** : le seul patch distinct de l’API serait le freeze.
...
2. **Garder le pulse.** Ne **pas** poser Size = API. Ne **pas** « fermer » trail / HUD / preview / select / flash / pulse / texte / gold / wake / splash / delivery / Shockwave / BlastSmoke (N151–N164 déjà). Ne **pas** lire `piece.part.Size` (alloc). Ne pas porter visual. Après N164. `BuildingModels` **non**. Overlay **non**. HUD **non**. Effects **non**. `createMissile` construction **non** (taille initiale `Vector3.new(0.62, 0.62, 2.3)` inchangée).

3. **Si le seul patch possible est un freeze Size=API : ne pas livrer N152. Laisser la ligne inchangée. Livrer N165 seulement.** N152 reste alors ouvert (feel-diverged vs V74). Ne pas inventer un cache Size. Ne pas skip-si-inchangé (N2, pas ici).

4. Tests « navires, missiles et interpolation » leftover N151 trail **et** leftover N148 mesh **et** leftover N147 Fin **doivent rester verts**. Tests « vagues de conquete » leftover N164 `BlastSmoke` reuse **et** leftover N163 `Shockwave` reuse **doivent rester verts**. Client **35/35**. `./tests/run.sh`. 6000 ticks serveur inchangé.

5. Fichiers : `UnitModels.luau` (`place` flame **seulement**, **un** `elseif`) **seulement si** un patch feel distinct de V74 existe. Sinon **aucun** fichier flame. `tests/client.luau` **seulement si** le check navires ne mentionne pas encore N152. Overlay **non**. **Ne pas** éditer le serveur. **Ne pas** toucher Shockwave ni BlastSmoke ni Blast sphère.

**Contraintes :** pas de RemoteFunction. **N152 feel ≠ N151 (trail Z) ≠ visual V74 (freeze Size=API, ne pas merger) ≠ N163 (Shockwave pool) ≠ N164 (BlastSmoke pool) ≠ N165 (Blast sphère pool) ≠ N2 (skip-si-inchangé replication).** Non réentrant. Ne pas fusionner avec N165 dans le même worker. Pulse Z **conservé** — ne pas figer à 1.8. Un `elseif` flame.

---

### ISSUE-N165 — Overlay Blast sphère Instance.new+Destroy chaque explosion (feel)

**Priorité :** P3 alloc client Overlay. Leftover explicite après N164 (BlastSmoke free-list déjà). Distinct de N152 (UnitModels Size), de N153–N163 (pools Overlay/Effects), de N164 (BlastSmoke pool `smokeFree`), de N163 (Shockwave pool `shockFree`). `Overlay.explosion` sphère `Blast` **seulement**. Ne pas toucher BlastSmoke. Ne pas toucher Shockwave. Ne pas toucher `routePart`. Ne pas toucher DeliveryPulse. Ne pas toucher LandingSplash. Ne pas toucher LaunchWake. Ne pas toucher `goldPopup`. Ne pas toucher HUD. Ne pas toucher UnitModels. Ne pas toucher Effects. Ne pas toucher `applyUnits`. Ne pas toucher `stepInterpolation`.

**Problème :** N164 ferme le pool BlastSmoke. N152 reste ouvert (freeze interdit). Reste, **chaque explosion** (`Overlay.explosion`, sphère Blast) :
...
-- ... tween Size (worldRadius * 2) / Transparency 1 ...
...
Instance.new Part + PointLight + Destroy à 1.3 s par frappe. BlastSmoke **réutilise déjà** (`smokeFree`, N164). Shockwave **réutilise déjà** (`shockFree`, N163). Blast non. Distinct de leftover N152 (`Size = Vector3.new` flamme). Distinct de leftover N164 (`smokeFree` **déjà** — **ne pas** partager). Distinct de leftover N163 (`shockFree` **déjà** — **ne pas** partager). Name `Blast` **inchangé**. Shape Ball **inchangé**. Material Neon **inchangé**. Size départ `Vector3.new(4, 4, 4)` **inchangé** (**réécrire** au reuse, le tween a fait `worldRadius * 2`). Color `Color3.fromRGB(255, 190, 110)` **inchangée**. Transparency départ `0.15` **inchangée**. CFrame `ground.X, 14, ground.Z` **inchangé** (**réécrire** : `ground` **varie**). Tween 1.1 Quart Out Size/Transparency **inchangé**. PointLight **conservé** (enfant de Blast — **réutiliser** `FindFirstChildWhichIsA("PointLight")` au reuse, reset Brightness `8` / Range `math.min(worldRadius * 2, 60)` / Color `255,200,120`, rejouer le Tween Brightness → 0 ; si absent : `Instance.new` comme aujourd’hui). `task.delay` 1.3 s **inchangé**. Garde Destroy devient `Parent = nil` + push si encore parenté. BlastSmoke / Shockwave **inchangés**. `routePart` **non modifié**.

**Pourquoi 20K CCU :** leftover N164. 8 clients × 474 explosions / 600 s × `Instance.new` Part + PointLight + Tween + Destroy 1.3 s. Pas d’autorité (cosmétique Overlay). BlastSmoke **déjà** N164 — ne pas y revenir. Shockwave **déjà** N163 — ne pas y revenir. Flame leftover N152 **alors** (ne pas freeze). BlastSmoke **doit** continuer (`smokeFree`). Shockwave **doit** continuer (`shockFree`). PointLight **doit** rester enfant de Blast (Destroy de la sphère emportait la lumière — au pool, `Parent = nil` **conserve** l’enfant). Oubli de reset Size / Transparency / CFrame / Range au reuse = sphère trop grosse (`worldRadius` change) et lumière trop faible (Brightness déjà 0).

**Worker :**

1. Dans `Overlay.explosion` sphère `Blast` seulement : free-list d’ancres (`blastFree` tableau, pop O(1)). **Pas** `smokeFree` (c’est Overlay N164). **Pas** `shockFree` (c’est Overlay N163). **Pas** `pulseFree` (c’est Effects N157). **Pas** `deliveryFree` (c’est Overlay N162). Si une Part libre : **réutiliser** (pas `Instance.new`). Writes **avant** Tween : `Size = Vector3.new(4, 4, 4)`, `Transparency = 0.15`, `Color = Color3.fromRGB(255, 190, 110)`, `CFrame = CFrame.new(ground.X, 14, ground.Z)`, `Parent = self.root`. Name `Blast` **conservé**. PointLight : `FindFirstChildWhichIsA("PointLight")` ; si présent reset Brightness/Range/Color **puis** Tween Brightness → 0 ; si absent : création inchangée (`Instance.new`, Parent = sphere). **Ne pas** Destroy à 1.3 s : `Parent = nil` + push free-list (`if sphere.Parent` devient push seulement si encore parenté — un second delay fantôme ne double-push pas). Sinon : création inchangée (`Instance.new` Part Name `Blast`, Ball, Neon, Size `(4,4,4)`). Tween Create sphère **conservé** (Size / Transparency restart — `worldRadius` **après** reset Size). Pas de `self.live`. **Ne pas** wrapper un record. **Ne pas** partager `smokeFree` / `shockFree` / `deliveryFree` / `splashFree` / `wakeFree` / `goldFree` / `textFree` / `pulseFree` / `flashFree`. **Ne pas** modifier `routePart`. **Ne pas** changer BlastSmoke / Shockwave / `smoke:Destroy()` (déjà pool). **Ne pas** toucher `applyUnits`. **Ne pas** toucher `stepInterpolation`. Un pop par explosion.

2. **Garder la sphère.** Ne **pas** recréer au reuse. Ne **pas** retirer Name `Blast`. Ne **pas** toucher BlastSmoke / N164. Ne **pas** toucher Shockwave / N163. Ne **pas** geler Size (pas N152). Après N164. Flame **non** (N152). HUD **non**. `UnitModels` **non**. Effects **non**. BlastSmoke **N164 conservé**. Shockwave **N163 conservé**. PointLight **enfant conservé** (pas un 2e pool).

3. Tests « navires, missiles et interpolation » leftover N164 BlastSmoke **sans** flush **et** leftover N163 Shockwave **sans** flush **et** leftover N133 euler **doivent rester verts** (`overlay:explosion(50, 50, 9)` **sans** `testFlushDelays`). Tests « vagues de conquete » leftover N164 `BlastSmoke` reuse **et** leftover N163 `Shockwave` reuse **doivent rester verts**. Client **35/35**. `./tests/run.sh`. 6000 ticks serveur inchangé.

4. Test : banc client `navires` **doit rester vert** (explosion **sans** flush, **pas** de `testFlushDelays`). Après flush des delays **dans le check vagues** (déjà un `testFlushDelays` — le Blast du check navires y tombe), `blastFree` `# >= 1` ; snapshot `pooledBlast` **avant** `applyUnits` N160 (comme N164 snapshot `smokeFree`) ; **ne pas** consommer avant `overlay:explosion(40, 40, 6)` N163/N164. Cette explosion (déjà dans vagues pour N163/N164) **consomme 1** Blast. Réparente (`rawequal` Parent root, Name `Blast`). **Réécrire** Size `(4,4,4)` + Transparency `0.15` + CFrame + PointLight Brightness/Range **avant** Tween — oubli = `worldRadius * 2` part de l’état tweené. Ne **pas** flush dans le check « navires ». **Ne pas** casser N164 (`smokeFree` snapshot + reuse 5 smokes `rawequal` Name `BlastSmoke`). **Ne pas** casser N163 (`shockFree` snapshot + reuse `rawequal` Name `Shockwave`). Assert `pooledBlast ~= pooledSmoke`. Assert `pooledBlast ~= pooledShock`. Check vagues leftover N164 / N163. Check navires leftover N133 euler / leftover N152 flame. Client **35/35**. `./tests/run.sh`. 6000 ticks serveur inchangé.

5. Fichiers : `Overlay.luau` (`explosion` sphère Blast **seulement**, PointLight enfant). `tests/client.luau` **seulement si** le check navires ne mentionne pas encore N165 (commentaire leftover, **garder** N164 BlastSmoke / N163 Shockwave / N133 euler). `Effects.luau` **non**. `HUD.luau` **non**. `UnitModels.luau` **non**. **Ne pas** éditer le serveur. **Ne pas** toucher flame ni Shockwave ni BlastSmoke ni DeliveryPulse. **Ne pas** modifier `routePart`. **Ne pas** changer le `Parent = nil` BlastSmoke / Shockwave.

**Contraintes :** pas de RemoteFunction. **N165 feel ≠ N164 (BlastSmoke pool) ≠ N163 (Shockwave pool) ≠ N152 (flame Size, ne pas freeze V74) ≠ PointLight pool séparé ≠ N2 (skip-si-inchangé replication).** Non réentrant. Ne pas fusionner avec N152 dans le même worker. Free-list — ne pas skip CFrame si `ground` change, ne pas skip Size/Range si `worldRadius` change. Un `explosion` — ne pas splitter Shockwave/fumée. `task.delay` 1.3 s **conservé**. Ne pas `table.remove` la free-list (pop O(1) depuis la fin). Ancre `Parent = nil` **pas** Destroy. Distinct `smokeFree` / `shockFree` / `deliveryFree` / `splashFree` / `wakeFree` / `goldFree` / `textFree` / `pulseFree` / `flashFree` — **ne pas** partager. **Reset Size + CFrame + Transparency + Color + PointLight Brightness/Range avant Tween** — oubli = sphère fantôme / trop grosse (`worldRadius` et `ground` varient). Name `Blast` **obligatoire**. PointLight **enfant obligatoire** (reuse, pas un 2e `blastFree`). Ne pas modifier `routePart`. BlastSmoke `smokeFree` **obligatoire**. Shockwave `shockFree` **obligatoire**. Nommer `blastFree` **pas** `smokeFree` **pas** `shockFree` **pas** `pulseFree`.
...
| N2 | Delta `stats` + UnitSnapshot dirty | P1 | ouvert (`buildPrices` → **N75 fait** ; … ; trail → **N151 fait** ; HUD feed → **N153 fait** ; previewTile → **N154 fait** ; selectTile → **N155 fait** ; tileFlash → **N156 fait** ; conquestPulse → **N157 fait** ; floatingText → **N158 fait** ; goldPopup → **N159 fait** ; LaunchWake → **N160 fait** ; LandingSplash → **N161 fait** ; DeliveryPulse → **N162 fait** ; Shockwave → **N163 fait** ; BlastSmoke → **N164 fait** ; reste skip-si-inchangé) |
...
| N10 | Divers P3 | P3 | ouvert (`Buildings.contextFor` → **N85 fait** ; … ; BlastSmoke pool → **N164** ; Overlay Blast sphère pool = **N165**) |
...
| N34–N151, N153–N164 | (voir rapport #204) | — | **faits** |
| N152 | UnitModels `place` flamme `Size = Vector3.new` 60 Hz | P3 | **ouvert** (`place`, pulse Z **conservé**, **≠** visual V74 freeze ; **non livré** passes 61–73) |
| N165 | Overlay Blast sphère Instance.new+Destroy | P3 | **nouveau** (`explosion` sphère free-list `blastFree`, Name `Blast` **conservé**, PointLight enfant, **≠** N164 `smokeFree` / N163 `shockFree`) |

Textes worker-ready N1–N25, N28, N33 : PR #21 / #22 / #24 / #26 / #29 / #32 / #34 / #36 / #38 / #41 / #42 / #45 / #48 / #51 / #53 / #56 / #59 / #62 / #65 / #68 / #71 / #75 / #78 / #82 / #86 / #89 / #93 / #96 / #99 / #101 / #106 / #108 / #111 / #114 / #118 / #121 / #125 / #128 / #131 / #133 / #136 / #140 / #144 / #147 / #150 / #153 / #155 / #158 / #161 / #163 / #165 / #167 / #169 / #171 / #173 / #176 / #178 / #181 / #183 / #185 / #187 / #189 / #191 / #193 / #195 / #197 / #200 / #202 / #204 `NIGHTLY_REPORT.md` historique.
...
| `TILE_SIZE` | 12 | n/a | oui (N101 lerp monde … N164 BlastSmoke pool) |
