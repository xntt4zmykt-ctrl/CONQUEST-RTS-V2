# Nightly report — passe 51 (revue PR #137)

**Branche revue :** `cursor/analyse-nocturne-du-codebase-b2f1` (PR #137, `7d732f6`)  
**Branche de correctifs :** `cursor/analyse-nocturne-du-codebase-0231`  
**Date :** 2026-08-26  
**Banc :** `./tests/run.sh` — serveur **vert**, client **34/34 vert**. `error()` si un invariant casse (Luau CLI sans `os.exit`).

Revue de PR #137 (`WorldCamera.step` offset `ox/oy/oz` nombres — HEAD visuel, V67). Correctifs sûrs, sans merger feel `bec6`/`ab04`/`846c` ni hardening `e9e5`/`e488`.
...
- **PR #137 :** offset caméra `ox/oy/oz` (V67) intact. Banc camera pitch=0 yaw=0 → `Z == focus.Z + distance` ; pitch 58° suit la formule. Unités yaw (V65) intact. Camion `segRot` (V64) intact. Rien à revert.

---

## Specs worker (reste)

Ne pas merger feel `bec6`/`ab04`/`846c` ni hardening `e9e5`/`e488` sur cette branche sans rebase. Porter **une** recette à la fois.
...
### ISSUE-V69 — WorldCamera champ `focusX/Y/Z` 60 Hz

**Problème.** V68 ferme le lerp Vector3. Reste, **une fois par frame** dans `WorldCamera.step` : `self.focus = Vector3.new(fx, fy, fz)`. Un Vector3 à l’écriture idle (aucune touche). Distinct de V68 (arithmétique lerp) : ici le **stockage**. `distance` / `yaw` / `pitch` sont déjà des nombres. `targetFocus` reste un Vector3 (écrit aux gestes, pas 60 Hz idle). `init.client` appelle `minimap:setFocus(camera.focus)` chaque RenderStepped.

**20K CCU.** Leftover V68. 8 clients × 60 Hz × 1 Vector3 idle + lecture `.X/.Y/.Z` depuis un userdata. Pas d’autorité (mire cosmétique). Un champ faux casserait le cap stratégique, le pincement tactile et le point minimap.

**Faire.**

1. Dans `WorldCamera` seulement : stocker `self.focusX` / `self.focusY` / `self.focusZ` (nombres, init `0`). Lerp et pose lisent/écrivent ces champs — plus de `Vector3.new` dans `step` overview. `focusTile(instant)` pose les trois nombres **et** `self.focus` (geste, pas idle). Après lerp, **ne pas** réécrire `self.focus` 60 Hz.

2. `init.client` RenderStepped : `m:setFocus(Vector3.new(camera.focusX, camera.focusY, camera.focusZ))` **ou** élargir `Minimap.setFocus` à `(x, y, z)` nombres. Un Vector3 côté minimap = leftover acceptable si c’est le **seul** (la minimap a déjà un indicateur Part). Préférer élargir `setFocus` pour zéro Vector3 caméra idle.

3. Ne **pas** splitter `targetFocus`. Ne pas « fermer » pan/clamp Vector3 (événements : `panByScreenDelta`, `clampFocus`, `clampToMap`). Ne pas porter Overlay unités (V65 déjà) ni camion (V64 déjà). Ne pas « fermer » `Size` chantier (API). Ne pas « fermer » `CFrame.Angles` roues camion (spin réel). Offset `ox/oy/oz` **inchangé** (V67). Shake `sx/sy/sz` **inchangé** (V66). Pose `CFrame.new(ex, ey, ez) * rotation` **inchangée** (V66). Lerp nombres **inchangé** (V68) — brancher `fx` sur `focusX`. Après V68.

**Contraintes.** Client-only. **V69 visual ≠ V68 (lerp) ≠ V67 (offset).** Non réentrant (une caméra). Lecteurs de `camera.focus` : `init.client` minimap + banc « camera strategique ». Adapter les deux dans le même commit. Client 34/34 (banc camera : pitch=0 yaw=0 → `Z == focusZ + distance` **doit rester vert** ; leftover tactile pincement/torsion **doit rester vert** ; leftover V68 lerp rattrape **doit rester vert** ; leftover V65 lerp `currentX`/`currentY` **doit rester vert** ; leftover V64 `segRot` `rawequal` **doit rester vert**). **Ne pas** éditer le serveur. Shake 0 : pose identique à V68. `alpha = 1 - math.exp(-12 * dt)` **inchangé**.

**Tester.** Banc client camera **doit rester vert** : overview `step(1/60)` ne lève pas ; leftover tactile pincement/torsion. Check navires leftover V65 (`currentX` **et** `currentY` avancent, premier `stepInterpolation` immobile) **et** leftover V55 (`targetX` mute) **et** leftover V52 (`extra` missile). Check pose/capture leftover V64 (`segRot` origine, `rawequal` après deux frames) **doit rester vert**. Check minimap `setFocus` après un `step` (indicateur suit `focusX/Z`). `./tests/run.sh`. Client 34/34.

**Fichiers.** `WorldCamera.luau` (champs + `step` overview + `focusTile` instant + `new`). `init.client.luau` (`setFocus` RenderStepped) **et/ou** `Minimap.luau` (`setFocus`). `tests/client.luau` **seulement si** le check « camera strategique » lit encore `camera.focus.X`. `Overlay.luau` **non**. `UnitModels.luau` **non**. `BuildingModels.luau` **non**.

---

## Hors scope volontaire

- Merger feel `bec6`/`ab04`/`846c` / hardening `e9e5`/`e488` sur #137/#134.
...
- Overlay unités lookAt deux Vector3 60 Hz — **fermé** (V65). V64 ne touche que le camion. Camera `lookAt` = **fermé** (V66). Offset `VectorToWorldSpace((0,0,distance))` = **fermé** (V67). Lerp `focus` Vector3 = **fermé** (V68). Champ `focusX/Y/Z` = leftover V69.
- Camera lookAt deux Vector3 60 Hz — **fermé** (V66). V65 ne touche que les unités Overlay. Offset `Vector3.new(0, 0, distance)` = **fermé** (V67). Lerp `focus` Vector3 = **fermé** (V68). Champ `focusX/Y/Z` = leftover V69.
- Camera offset `Vector3.new(0, 0, distance)` 60 Hz — **fermé** (V67). V66 ne touche que lookAt + shake nombres. Lerp `focus` Vector3 = **fermé** (V68). Champ `focusX/Y/Z` = leftover V69.
- Camera lerp `focus` Vector3 60 Hz — **fermé** (V68). V67 ne touche que l’offset nombres. Champ `focusX/Y/Z` = leftover V69.
- Camera champ `focusX/Y/Z` 60 Hz — **ouvert** (V69). V68 ne touche que le lerp nombres. `targetFocus` Vector3 et pan/clamp = événements, leftover séparé.
- `UnitModels.place` `Vector3.new` Size flamme missile — leftover séparé (API Size, ne pas éditer UnitModels dans V69). Radar / flag unité `CFrame.Angles` = rotation réelle.
...
Serveur : invariants + P0 + or plat + `removePlayer` refund + embargo auto + cap 3 transports + passe 16–50 inchangées (passe 51 = client-only).  
...
Client V67 : même check camera — pitch=0 yaw=0 → `CFrame.X/Y == focus`, `CFrame.Z == focus.Z + distance` ; pitch 58° suit `ox/oy/oz` (plus de `Vector3.new(0,0,d)` / `VectorToWorldSpace`). Leftover tactile pincement/torsion **et** leftover V65 (navires) **et** leftover V64 (segRot pose/capture) **et** leftover V66 (pose) **doivent rester verts**.  
Client V68 : même check camera — `targetFocus = focus + (40,0,0)` puis `step(1/60)` → `focus.X` avance sans sauter ; pose utilise `fx/fy/fz` ; Y/Z inchangés. Leftover V67 (offset) **et** leftover tactile **et** leftover V65 **et** leftover V64 **doivent rester verts**.  
