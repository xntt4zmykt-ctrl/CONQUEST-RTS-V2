# Nightly report — passe 67 (revue PR #174)

**Branche revue :** `cursor/analyse-nocturne-du-codebase-8015` (PR #174, `85f84ac`)  
**Branche de correctifs :** `cursor/analyse-nocturne-du-codebase-7be5`  
**Date :** 2026-08-26  
**Banc :** `./tests/run.sh` — serveur **vert**, client **34/34 vert**. `error()` si un invariant casse (Luau CLI sans `os.exit`).

Revue de PR #174 (`WorldRenderer` OceanGlint `fromEulerAnglesYXZ` — HEAD visuel, V84). Correctifs sûrs, sans merger feel `595e`/`c299`/`bfcc` ni hardening `41e2`/`93f6`.
...
- **PR #174 :** V84 intact (`fromEulerAnglesYXZ` yaw `angle` variable, Y = `OCEAN_LEVEL + 0.08`, `base` après pose). SelectionRing V83 / ConquestPulse V82 / Shockwave V81 / DeliveryPulse V80 / LandingSplash V79 / LaunchWake V78 / early-out V77 / Size V76 / rot V75 **non retouchés**. Rien à revert.

---

## Specs worker (reste)

Ne pas merger feel `595e`/`c299`/`bfcc` ni hardening `41e2`/`93f6` sur cette branche sans rebase. Porter **une** recette à la fois. Feel N129 (footprint hauteur **0.4**, **pas** de pulse) ≠ visual V75/V76/V77 — ne pas merger. Feel N134 `Y = 3` **non** porté (visuel = surface + 0.8). Feel N135 OceanGlint **fermée** (V84) — **ne pas merger** `595e`. Feel N136 BuildRing **fermée** ici (V85 inline) — **ne pas merger** `595e`. Feel N137 TreeTrunk — **ne pas merger** `c299`.
...
### ISSUE-V86 — WorldRenderer `buildDecorations` TreeTrunk / SavannaTrunk `CFrame.Angles`

**Problème.** V85 ferme `CFrame.Angles` à **l’animation de chantier** (`playConstruction` / `BuildRing`, Z=90° fixe). Reste, à **la construction des décorations** (`WorldRenderer.buildDecorations`) :

```
CFrame.new(base.X, ground + height / 2, base.Z) * CFrame.Angles(0, 0, math.rad(90))
CFrame.new(base.X, ground + 1.6, base.Z) * CFrame.Angles(0, 0, math.rad(90))
```

Deux sites : `TreeTrunk` (FOREST, cylindre, hauteur variable) et `SavannaTrunk` (SAVANNA, cylindre, Y = `ground + 1.6`). Z **fixe** 90° (cylindre Roblox pointe X, on redresse). Distinct de V85 (BuildRing événement chantier, `BuildingModels`). Distinct de V84 (OceanGlint yaw `angle` **variable**, `buildOcean`). Distinct de V59 (feuillage 60 Hz, Y nombres depuis `base`, **pas** la pose du tronc). Distinct de V58 (houle 60 Hz). Construction de chunk, **pas** 60 Hz interpolation. `Shape = Cylinder` **déjà** posé après. Couronne `TreeCrown` / `SavannaCrown` **sans** Angles (translation seule) — **ne pas** y toucher. `animatedFoliage` enregistre la couronne, **pas** le tronc.

**20K CCU.** 8 clients × rebuild décorations (chargement + chunks). Pas d’autorité. Même classe que feel N137 (Angles → `fromEulerAnglesYXZ` inline, Z=90°, **deux** sites — **ne pas** cuire un rot, yaw n’est pas variable ici).

**Faire.** Remplacer les deux `CFrame.Angles(0, 0, math.rad(90))` par `CFrame.fromEulerAnglesYXZ(0, 0, math.rad(90))`. Hot path TreeTrunk : `CFrame.new(base.X, ground + height / 2, base.Z) * CFrame.fromEulerAnglesYXZ(0, 0, math.rad(90))`. Hot path SavannaTrunk : `CFrame.new(base.X, ground + 1.6, base.Z) * CFrame.fromEulerAnglesYXZ(0, 0, math.rad(90))`. `height` / `base` / `ground` / Size / Color / Material / Name **inchangés**. `Shape = Cylinder` **inchangé**. Ne **pas** cuire un `trunkRot` (Z fixe mais translations distinctes, recette feel N137 **inline**). Ne **pas** toucher `Rock` (leftover V87, yaw `phase` variable). Ne **pas** changer `WorldRenderer.step` (V58/V59 déjà). Ne **pas** changer `BuildingModels.luau` (V85 déjà). Ne **pas** changer `Effects.luau`. Ne **pas** merger feel `c299`.

**Contraintes.** Client-only. **V86 visual ≠ V85 (BuildRing) ≠ V84 (OceanGlint) ≠ V59 (feuillage 60 Hz).** Recette feel N137 — **ne pas merger** `c299`. Client 34/34 (check construction du monde **doit rester vert**. Check modeles V85 **doit rester vert**. Check calques V83 **doit rester vert**. Check vagues V82 **doit rester vert**. V58 houle **doit rester vert**. V59 feuillage **doit rester vert**). **Ne pas** éditer le serveur. **Ne pas** éditer `BuildingModels.luau`. **Ne pas** éditer `Effects.luau`. **Ne pas** éditer `Overlay.luau`. **Ne pas** éditer `UnitModels.luau`. **Ne pas** éditer `buildOcean`. **Ne pas** éditer `Rock`.

**Tester.** Check « construction du monde 3D » : après rebuild, au moins un Part `Name == TreeTrunk` ou `SavannaTrunk` dans `world.decorations` ; tronc `Shape` cylindre. V84 OceanGlint Y = `OCEAN_LEVEL + 0.08` **doit rester vert**. V58 (Part glint stable, X/Z bougent) **doit rester vert**. V59 (couronne Y bouge) **doit rester vert**. Check modeles V85 (`BuildRing` Y == 2) **doit rester vert**. `./tests/run.sh`. Client 34/34.

**Fichiers.** `WorldRenderer.luau` (`buildDecorations` **seulement**, les deux lignes `TreeTrunk` / `SavannaTrunk`). `tests/client.luau` check construction + commentaire leftover. `BuildingModels.luau` **non**. `Effects.luau` **non**. `Overlay.luau` **non**. `playConstruction` **non**. `buildOcean` **non**. `Rock` **non**. `init.client.luau` **non**.

---

## Hors scope volontaire

- Merger feel `595e`/`c299`/`bfcc` / hardening `41e2`/`93f6` sur #174.
...
- Feel N136 BuildRing — **porté ici** (V85 inline, Z=90° fixe, **pas** merger `595e`).
...
- WorldRenderer `buildDecorations` TreeTrunk / SavannaTrunk `CFrame.Angles` — leftover V86 (Z=90° fixe, deux sites, `fromEulerAnglesYXZ` inline, recette feel N137, **pas** un rot cuit, **pas** merger `c299`).
- `CFrame.Angles` de construction restants (`addWake`, `Bow`, ailettes, `Rock` yaw `phase`, toits usine, portes silo, rampes SAM, `cylinder()`) — une fois, pas 60 Hz interpolation. Prochaine après V86 = `Rock` (feel N138, yaw `phase` variable).
...
Serveur : invariants + P0 + or plat + `removePlayer` refund + embargo auto + cap 3 transports + passe 16–66 inchangées (passe 67 = client-only).  
