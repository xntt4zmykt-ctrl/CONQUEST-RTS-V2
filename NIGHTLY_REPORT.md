...
**PR #197 (passe 69) : claims vérifiés.** Overlay LaunchWake free-list (N160, `wakeFree` pop O(1), `Parent = nil` + push si encore parenté, pas Destroy, Name `LaunchWake` conservé, euler N130 inline). **N152 non livré** (freeze Size=API = visual V74, interdit). Spec N161 **corrigée ici**. Combat vivant = `ChantierB.stepAttacks`. `MAX_TILES_PER_TICK` non lu par le combat installé. Visual **PR #198** (`ae72`) a **fermé V97** mesh carrier — feel N148 **déjà**, **pas merger** `ae72`. Visual **PR #199** (`c94d`) a **fermé V98** CityWindows — feel N149 **déjà**, **pas merger** `c94d`. Visual V99 beacons **ouvert** — feel N150 **déjà**, **pas merger**. Visual **PR #196** (`f89f`) V96 Fin **fermé** — feel N147 **déjà**. Visual **PR #194** (`60f2`) V95 MissileBody **fermé** — feel N146 **déjà**.
...
PRs ouvertes au moment de la revue : #16 P0, hardening jusqu’à #160/`41e2` (N107–N108), feel jusqu’à #197, visuelles #39/…/`ae72` V97 **fermé** / `c94d` V98 **fermé** / leftover V99. **#197 + cette passe** est le sur-ensemble feel à merger. La ligne P0 sans feel reste distincte. Ne pas merger visual `c94d` / `ae72` / `f89f` / `60f2` / `aae2` / `29b4` / `d645` / `930a` / `6095` / `9fd4` / `f9ec` / `2b04` / `75ce` / `7be5` / `0b3d` / `490f` / `47c0` / `a597` / `54d6` / `6183` / `185a` / `70a5` / `c0ec` / `1b5c` ni hardening `41e2` / `93f6` / `e291` sans rebase.
