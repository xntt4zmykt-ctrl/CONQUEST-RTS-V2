# Nightly report — passe 72 (revue PR #182)

**Branche revue :** `cursor/analyse-nocturne-du-codebase-9fd4` (PR #182, `b5df35f`)  
**Branche de correctifs :** `cursor/analyse-nocturne-du-codebase-6095`  
**Date :** 2026-08-26  
**Banc :** `./tests/run.sh` — serveur **vert**, client **34/34 vert**. `error()` si un invariant casse (Luau CLI sans `os.exit`).

Revue de PR #182 (`BuildingModels` `buildFactory` toits `fromEulerAnglesYXZ` — HEAD visuel, V89). Correctifs sûrs, sans merger feel `4885`/`a8e1`/`c299`/`595e`/`bfcc` ni hardening `41e2`/`93f6`.
...
- **PR #182 :** V89 intact (`fromEulerAnglesYXZ` Z=-12° fixe, toits `buildFactory`, pas de `roofRot`). Portes V90 **fermées ici**. `cylinder()` V88 / Rock V87 / TreeTrunk V86 / BuildRing V85 / OceanGlint V84 / SelectionRing V83 / ConquestPulse V82 / Shockwave V81 / DeliveryPulse V80 / LandingSplash V79 / LaunchWake V78 / early-out V77 / Size V76 / rot V75 **non retouchés**. Rien à revert.

---

## Specs worker (reste)

Ne pas merger feel `4885`/`a8e1`/`c299`/`595e`/`bfcc` ni hardening `41e2`/`93f6` sur cette branche sans rebase. Porter **une** recette à la fois. Feel N129 (footprint hauteur **0.4**, **pas** de pulse) ≠ visual V75/V76/V77 — ne pas merger. Feel N134 `Y = 3` **non** porté (visuel = surface + 0.8). Feel N135 OceanGlint **fermée** (V84) — **ne pas merger** `595e`. Feel N136 BuildRing **fermée** (V85, PR #175) — **ne pas merger** `595e`. Feel N137 TreeTrunk **fermée** (V86, PR #177) — **ne pas merger** `c299`. Feel N138 Rock **fermée** (V87, PR #179) — **ne pas merger** `c299`. Feel N139 `cylinder()` **fermée** (V88, PR #180) — **ne pas merger** `a8e1`. Feel N140 toits usine **fermée** (V89, PR #182) — **ne pas merger** `a8e1`. Feel N141 portes silo **fermée** ici (V90 inline) — **ne pas merger** `4885`. Feel N142 rampes SAM — **ne pas merger** `4885`.
...
### ISSUE-V91 — BuildingModels `buildSam` rampes `CFrame.Angles`

**Problème.** V90 ferme `CFrame.Angles` sur les **deux portes** de `buildSilo` (Z=±18° fixe, `BuildingModels`). Restent les **quatre Interceptor** du SAM dans `buildSam` :

```
launcher.CFrame = CFrame.new(side * 1.45, 5.1, lane * 0.72) * CFrame.Angles(math.rad(-31), 0, 0)
```

Pitch X **fixe** -31° (rampes inclinées, 2×2 via `side`/`lane` ∈ {-1,1}). `Name = Interceptor`. Distinct de V90 (portes silo, Z=±18°, **même fichier**, `buildSilo` — ne **pas** retoucher les portes). Distinct de V89 (toits usine, Z=-12°, `buildFactory` — ne **pas** retoucher les toits). Distinct de V88 (`cylinder()` helper, Z=90° — ne **pas** retoucher le helper). Distinct de leftover V92 (`UnitModels.addWake` yaw Y=±11°). Construction de modèle, **pas** 60 Hz interpolation. `block()` pose d’abord `CFrame.new(side * 1.45, 5.1, lane * 0.72)` ; la ligne **écrase** ensuite. MISSILE / Size `0.72, 5.8, 0.72` / Material Metal **inchangés**. Nez Neon **inchangé**.

**20K CCU.** 8 clients × pose SAM (chargement + capture + upgrade palier). Pas d’autorité. Même classe que feel N142 (Angles → `fromEulerAnglesYXZ` inline, pitch X=-31° — **ne pas** cuire un rot, translations `side`/`lane` distinctes, comme V86 `trunkRot` / V88 `cylinderRot` / V89 `roofRot` / V90 `doorRot` interdits).

**Faire.** Remplacer `CFrame.Angles(math.rad(-31), 0, 0)` par `CFrame.fromEulerAnglesYXZ(math.rad(-31), 0, 0)`. Hot path : `launcher.CFrame = CFrame.new(side * 1.45, 5.1, lane * 0.72) * CFrame.fromEulerAnglesYXZ(math.rad(-31), 0, 0)`. Position / Size / Color / Material / Name **inchangés**. Ne **pas** cuire un `rampRot` (translations `side`/`lane` distinctes, recette feel N142 **inline**). Ne **pas** retoucher `cylinder()` (V88 déjà). Ne **pas** éditer les toits `buildFactory` (V89 déjà). Ne **pas** éditer les portes `buildSilo` (V90 déjà). Ne **pas** éditer le radar / socle / nez. Ne **pas** changer `BuildingModels.animate` (V60/V70 déjà). Ne **pas** changer `playConstruction` (V85 déjà). Ne **pas** changer `WorldRenderer.luau` (V86/V87 déjà). Ne **pas** merger feel `4885`.

**Contraintes.** Client-only. **V91 visual ≠ V90 (portes Z=±18°) ≠ V89 (toits Z=-12°) ≠ V88 (`cylinder()` Z=90°) ≠ V87 (Rock yaw `phase`) ≠ V92 leftover (`addWake` yaw Y=±11°).** Recette feel N142 — **ne pas merger** `4885`. Client 34/34 (check modeles V90 portes `# == 2` **doit rester vert**. Check V89 toits palier 1 **doit rester vert**. Check V88 Chimney cylindre **doit rester vert**. Check silo StrategicMissile cylindre **doit rester vert**. Check construction V87 Rock WedgePart **doit rester vert**. Check TreeTrunk V86 cylindre **doit rester vert**. Check BuildRing V85 Y == 2 **doit rester vert**). **Ne pas** éditer le serveur. **Ne pas** éditer `WorldRenderer.luau`. **Ne pas** éditer `Effects.luau`. **Ne pas** éditer `Overlay.luau`. **Ne pas** éditer `UnitModels.luau`. **Ne pas** éditer `cylinder()`. **Ne pas** éditer `buildFactory`. **Ne pas** éditer `buildSilo`. **Ne pas** éditer `playConstruction`.

**Tester.** Check « modeles procéduraux : le palier change la silhouette » : après `Building.create(SAM, …)`, quatre `Interceptor` présents, `Y == 5.1 * WORLD_SCALE`. V90 portes `# == 2` Y == 1.3 × scale **doit rester vert**. V89 toits `# == 2` palier 1 **doit rester vert**. V88 Chimney / StrategicMissile **doit rester vert**. V85 BuildRing Y == 2 **doit rester vert**. `./tests/run.sh`. Client 34/34.

**Fichiers.** `BuildingModels.luau` (`buildSam` **seulement** la ligne CFrame des Interceptor). `tests/client.luau` check modeles + commentaire leftover. `cylinder()` **non**. `buildFactory` **non**. `buildSilo` **non**. `WorldRenderer.luau` **non**. `Effects.luau` **non**. `Overlay.luau` **non**. `playConstruction` **non**. `init.client.luau` **non**.

---

## Hors scope volontaire

- Merger feel `4885`/`a8e1`/`c299`/`595e`/`bfcc` / hardening `41e2`/`93f6` sur #182.
...
- Feel N141 portes silo — **porté ici** (V90 inline, Z=±18° fixe, **pas** merger `4885`).
- Feel N142 rampes SAM — leftover V91 (pitch X=-31°, **pas** merger `4885`).
...
- BuildingModels `buildSam` rampes `CFrame.Angles` — leftover V91 (pitch X=-31° fixe, 2×2 Interceptor, recette feel N142 inline, **pas** un rot cuit, **pas** merger `4885`). Portes **non** retouchées.
- `CFrame.Angles` de construction restants (`addWake`, `Bow`, ailettes, rampes SAM) — une fois, pas 60 Hz interpolation. Prochaine après V91 = UnitModels `addWake` (feel N143, yaw Y=±11°).
...
Serveur : invariants + P0 + or plat + `removePlayer` refund + embargo auto + cap 3 transports + passe 16–71 inchangées (passe 72 = client-only).  
...
Client V90 : même check modeles — 2 `SiloDoor`, `Y == 1.3 * WORLD_SCALE`. V89 toits `# == 2` palier 1 **doit rester vert**. V88 Chimney / StrategicMissile **doit rester vert**. V85 BuildRing Y == 2 **doit rester vert**. Leftover V91 rampes SAM **non testé ici**.  
