# Nightly report — passe 70 (revue PR #179)

**Branche revue :** `cursor/analyse-nocturne-du-codebase-2b04` (PR #179, `fe5a87b`)  
**Branche de correctifs :** `cursor/analyse-nocturne-du-codebase-f9ec`  
**Date :** 2026-08-26  
**Banc :** `./tests/run.sh` — serveur **vert**, client **34/34 vert**. `error()` si un invariant casse (Luau CLI sans `os.exit`).

Revue de PR #179 (`WorldRenderer` Rock `fromEulerAnglesYXZ` — HEAD visuel, V87). Correctifs sûrs, sans merger feel `a8e1`/`c299`/`595e`/`bfcc` ni hardening `41e2`/`93f6`.
...
- **PR #179 :** V87 intact (`fromEulerAnglesYXZ` yaw `phase` variable, Rock WedgePart, pas de `rockRot`). `cylinder()` V88 **fermé ici**. TreeTrunk V86 / BuildRing V85 / OceanGlint V84 / SelectionRing V83 / ConquestPulse V82 / Shockwave V81 / DeliveryPulse V80 / LandingSplash V79 / LaunchWake V78 / early-out V77 / Size V76 / rot V75 **non retouchés**. Rien à revert.

---

## Specs worker (reste)

Ne pas merger feel `a8e1`/`c299`/`595e`/`bfcc` ni hardening `41e2`/`93f6` sur cette branche sans rebase. Porter **une** recette à la fois. Feel N129 (footprint hauteur **0.4**, **pas** de pulse) ≠ visual V75/V76/V77 — ne pas merger. Feel N134 `Y = 3` **non** porté (visuel = surface + 0.8). Feel N135 OceanGlint **fermée** (V84) — **ne pas merger** `595e`. Feel N136 BuildRing **fermée** (V85, PR #175) — **ne pas merger** `595e`. Feel N137 TreeTrunk **fermée** (V86, PR #177) — **ne pas merger** `c299`. Feel N138 Rock **fermée** (V87, PR #179) — **ne pas merger** `c299`. Feel N139 `cylinder()` **fermée** ici (V88 inline) — **ne pas merger** `a8e1`. Feel N140 toits usine — **ne pas merger** `a8e1`.
...
### ISSUE-V89 — BuildingModels `buildFactory` toits `CFrame.Angles`

**Problème.** V88 ferme `CFrame.Angles` sur le helper mesh **`cylinder()`** (Z=90° fixe, `BuildingModels`). Reste la boucle des **toits sheds** dans `buildFactory` :

```
roof.CFrame = CFrame.new(x, roofY, 0) * CFrame.Angles(0, 0, math.rad(-12))
```

Z **fixe** -12° (pente des dents de scie). `teeth = 1 + tier` ; `x` et `roofY` varient par palier. Distinct de V88 (`cylinder()` helper, Z=90°, **même fichier**, **même** `buildFactory` — ne **pas** retoucher le helper). Distinct de V87 (Rock yaw `phase` variable, `WorldRenderer`). Distinct de V86 (TreeTrunk, Z=90°, `WorldRenderer`). Distinct de V85 (BuildRing, Z=90°, `playConstruction`). Distinct de V84 (OceanGlint yaw `angle` variable). Construction de modèle, **pas** 60 Hz interpolation. `block()` pose d’abord `CFrame.new(x, roofY, 0)` ; la ligne **écrase** ensuite. STEEL / Size `2.3, 0.35, 7` / Material Metal **inchangés**.

**20K CCU.** 8 clients × pose usine (chargement + capture + upgrade palier). Pas d’autorité. Même classe que feel N140 (Angles → `fromEulerAnglesYXZ` inline, Z=-12° fixe — **ne pas** cuire un rot, `x` distinct par dent, comme V86 `trunkRot` / V88 `cylinderRot` interdits).

**Faire.** Remplacer `CFrame.Angles(0, 0, math.rad(-12))` par `CFrame.fromEulerAnglesYXZ(0, 0, math.rad(-12))`. Hot path : `roof.CFrame = CFrame.new(x, roofY, 0) * CFrame.fromEulerAnglesYXZ(0, 0, math.rad(-12))`. `teeth` / `x` / `roofY` / Size / Color / Material **inchangés**. Ne **pas** cuire un `roofRot` (translations distinctes, recette feel N140 **inline**). Ne **pas** retoucher `cylinder()` (V88 déjà). Ne **pas** éditer les cheminées / réservoir. Ne **pas** toucher les portes silo (leftover V90, Z=±18°). Ne **pas** changer `BuildingModels.animate` (V60/V70 déjà). Ne **pas** changer `playConstruction` (V85 déjà). Ne **pas** changer `WorldRenderer.luau` (V86/V87 déjà). Ne **pas** merger feel `a8e1`.

**Contraintes.** Client-only. **V89 visual ≠ V88 (`cylinder()` Z=90°) ≠ V87 (Rock yaw `phase`) ≠ V86 (TreeTrunk) ≠ V85 (BuildRing).** Recette feel N140 — **ne pas merger** `a8e1`. Client 34/34 (check modeles V88 Chimney cylindre **doit rester vert**. Check silo StrategicMissile cylindre **doit rester vert**. Check construction V87 Rock WedgePart **doit rester vert**. Check TreeTrunk V86 cylindre **doit rester vert**. Check BuildRing V85 Y == 2 **doit rester vert**). **Ne pas** éditer le serveur. **Ne pas** éditer `WorldRenderer.luau`. **Ne pas** éditer `Effects.luau`. **Ne pas** éditer `Overlay.luau`. **Ne pas** éditer `UnitModels.luau`. **Ne pas** éditer `cylinder()`. **Ne pas** éditer `buildSilo`. **Ne pas** éditer `playConstruction`.

**Tester.** Check « modeles procéduraux : le palier change la silhouette » : après `Building.create(FACTORY, …)`, `Chimney` cylindre **reste** ; silhouette palier 1 vs 3 **reste verte** (`highTop > lowTop + 0.5`). V88 Chimney / StrategicMissile **doit rester vert**. V85 BuildRing Y == 2 **doit rester vert**. `./tests/run.sh`. Client 34/34.

**Fichiers.** `BuildingModels.luau` (`buildFactory` **seulement** la ligne CFrame des toits). `tests/client.luau` check modeles + commentaire leftover. `cylinder()` **non**. `WorldRenderer.luau` **non**. `Effects.luau` **non**. `Overlay.luau` **non**. `playConstruction` **non**. `buildSilo` **non**. `init.client.luau` **non**.

---

## Hors scope volontaire

- Merger feel `a8e1`/`c299`/`595e`/`bfcc` / hardening `41e2`/`93f6` sur #179.
...
- Feel N139 `cylinder()` — **porté ici** (V88 inline, Z=90° fixe, **pas** merger `a8e1`).
...
- BuildingModels `buildFactory` toits `CFrame.Angles` — leftover V89 (Z=-12° fixe, dents de scie, recette feel N140 inline, **pas** un rot cuit, **pas** merger `a8e1`). `cylinder()` **non** retouché.
- `CFrame.Angles` de construction restants (`addWake`, `Bow`, ailettes, portes silo, rampes SAM) — une fois, pas 60 Hz interpolation. Prochaine après V89 = portes silo (feel N141, Z=±18°, `buildSilo`).
...
Serveur : invariants + P0 + or plat + `removePlayer` refund + embargo auto + cap 3 transports + passe 16–69 inchangées (passe 70 = client-only).  
...
Client V88 : check « modeles procéduraux » — après `Building.create(FACTORY, …)`, `Chimney` présent, `Shape == Enum.PartType.Cylinder` ; après `Building.create(SILO, …)`, `StrategicMissile` présent, `Shape == Enum.PartType.Cylinder`. V85 BuildRing Y == 2 **doit rester vert**. V87 Rock WedgePart **doit rester vert**. Leftover V89 toits **non testé ici**.  
