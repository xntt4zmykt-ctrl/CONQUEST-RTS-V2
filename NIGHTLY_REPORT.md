# Nightly report — passe 73 (revue PR #184)

**Branche revue :** `cursor/analyse-nocturne-du-codebase-6095` (PR #184, `3e2aeb4`)  
**Branche de correctifs :** `cursor/analyse-nocturne-du-codebase-930a`  
**Date :** 2026-08-26  
**Banc :** `./tests/run.sh` — serveur **vert**, client **34/34 vert**. `error()` si un invariant casse (Luau CLI sans `os.exit`).

Revue de PR #184 (`BuildingModels` `buildSilo` portes `fromEulerAnglesYXZ` — HEAD visuel, V90). Correctifs sûrs, sans merger feel `4885`/`a8e1`/`c299`/`595e`/`bfcc` ni hardening `41e2`/`93f6`.
...
- **PR #184 :** V90 intact (`fromEulerAnglesYXZ` Z=±18° fixe, portes `buildSilo`, pas de `doorRot`). Rampes V91 **fermées ici**. Toits V89 / `cylinder()` V88 / Rock V87 / TreeTrunk V86 / BuildRing V85 / OceanGlint V84 / SelectionRing V83 / ConquestPulse V82 / Shockwave V81 / DeliveryPulse V80 / LandingSplash V79 / LaunchWake V78 / early-out V77 / Size V76 / rot V75 **non retouchés**. Rien à revert.

---

## Specs worker (reste)

Ne pas merger feel `4885`/`a8e1`/`c299`/`595e`/`bfcc`/`c62f` ni hardening `41e2`/`93f6` sur cette branche sans rebase. Porter **une** recette à la fois. Feel N129 (footprint hauteur **0.4**, **pas** de pulse) ≠ visual V75/V76/V77 — ne pas merger. Feel N134 `Y = 3` **non** porté (visuel = surface + 0.8). Feel N135 OceanGlint **fermée** (V84) — **ne pas merger** `595e`. Feel N136 BuildRing **fermée** (V85, PR #175) — **ne pas merger** `595e`. Feel N137 TreeTrunk **fermée** (V86, PR #177) — **ne pas merger** `c299`. Feel N138 Rock **fermée** (V87, PR #179) — **ne pas merger** `c299`. Feel N139 `cylinder()` **fermée** (V88, PR #180) — **ne pas merger** `a8e1`. Feel N140 toits usine **fermée** (V89, PR #182) — **ne pas merger** `a8e1`. Feel N141 portes silo **fermée** (V90, PR #184) — **ne pas merger** `4885`. Feel N142 rampes SAM **fermée** ici (V91 inline) — **ne pas merger** `4885`. Feel N143 `addWake` — **ne pas merger** `c62f`.
...
### ISSUE-V92 — UnitModels `addWake` `CFrame.Angles`

**Problème.** V91 ferme `CFrame.Angles` sur les **quatre Interceptor** de `buildSam` (pitch X=-31° fixe, `BuildingModels`). Restent les **deux sillages** navire dans `UnitModels.addWake` :

```
CFrame.new(side * 2.15, -0.68, 5.5) * CFrame.Angles(0, math.rad(side * 11), 0)
```

Yaw Y **fixe** ±11° (deux sides, `side` ∈ {-1,1}). `Name = Wake`. Distinct de V91 (rampes SAM, pitch X=-31°, `BuildingModels` — ne **pas** retoucher les Interceptor). Distinct de leftover V93 (`Bow` yaw Y=π, carrier + transport). Distinct de V78 (LaunchWake Overlay, `wakeRot` cuit, Y = OCEAN+0.12 — **autre objet**). Construction de modèle, **pas** 60 Hz interpolation. `addPart` pose `part.CFrame = offset` ; Overlay `place` compose `frame * offset` ensuite. Size `1.15, 0.08, 6.5` / FOAM / Neon / Transparency 0.62 / `role = "wake"` **inchangés**. Call sites `createBoat` kind==3 (carrier) **et** kind==1 (transport, ligne après `buildTransport`) **non édités** hors le helper.

**20K CCU.** 8 clients × pose navire (spawn + capture de côte). Pas d’autorité. Même classe que feel N143 (Angles → `fromEulerAnglesYXZ` inline, yaw Y=±11° — **ne pas** cuire un rot, translations `side` distinctes, comme V86 `trunkRot` / V88 `cylinderRot` / V89 `roofRot` / V90 `doorRot` / V91 `rampRot` interdits).

**Faire.** Remplacer `CFrame.Angles(0, math.rad(side * 11), 0)` par `CFrame.fromEulerAnglesYXZ(0, math.rad(side * 11), 0)`. Hot path : `CFrame.new(side * 2.15, -0.68, 5.5) * CFrame.fromEulerAnglesYXZ(0, math.rad(side * 11), 0)`. Position / Size / Color / Material / Name / Transparency **inchangés**. Ne **pas** cuire un `wakeRot` (signes Y opposés + translations `side` distinctes, recette feel N143 **inline** — **≠** Overlay `self.wakeRot` V78). Ne **pas** retoucher `BuildingModels` (V88–V91 déjà). Ne **pas** éditer `Bow` (leftover V93). Ne **pas** éditer `LandingRamp` / `MissileBody` / `Fin`. Ne **pas** changer `UnitModels.place` (V73 déjà). Ne **pas** changer Overlay (V71/V72/V78 déjà). Ne **pas** merger feel `c62f`.

**Contraintes.** Client-only. **V92 visual ≠ V91 (Interceptor pitch X=-31°) ≠ V90 (portes Z=±18°) ≠ V78 (LaunchWake Overlay) ≠ V93 leftover (`Bow` Y=π).** Recette feel N143 — **ne pas merger** `c62f`. Client 34/34 (check modeles V91 Interceptor `# == 4` Y == 5.1 × scale **doit rester vert**. Check V90 portes `# == 2` **doit rester vert**. Check V89 toits palier 1 **doit rester vert**. Check V88 Chimney cylindre **doit rester vert**. Check navires V78 LaunchWake **doit rester vert**). **Ne pas** éditer le serveur. **Ne pas** éditer `BuildingModels.luau`. **Ne pas** éditer `WorldRenderer.luau`. **Ne pas** éditer `Effects.luau`. **Ne pas** éditer `Overlay.luau`. **Ne pas** éditer `Bow` / `LandingRamp` / `MissileBody`.

**Tester.** Check « navires, missiles et interpolation » : après spawn 3 navires, chaque modèle navire a 2 Parts `Name == Wake`. Offset local `Y == -0.68` (construction ; Overlay `place` compose ensuite). Distinct de LaunchWake Overlay (V78, Y = OCEAN+0.12). V91 Interceptor `# == 4` Y == 5.1 × scale **doit rester vert**. V90 portes `# == 2` **doit rester vert**. V85 BuildRing Y == 2 **doit rester vert**. `./tests/run.sh`. Client 34/34.

**Fichiers.** `UnitModels.luau` (`addWake` **seulement** la ligne CFrame des Wake). `tests/client.luau` check navires + commentaire leftover. `BuildingModels.luau` **non**. `buildSam` **non**. `Bow` **non**. `WorldRenderer.luau` **non**. `Effects.luau` **non**. `Overlay.luau` **non**. `init.client.luau` **non**.

---

## Hors scope volontaire

- Merger feel `4885`/`a8e1`/`c299`/`595e`/`bfcc`/`c62f` / hardening `41e2`/`93f6` sur #184.
...
- Feel N142 rampes SAM — **porté ici** (V91 inline, pitch X=-31° fixe, **pas** merger `4885`).
- Feel N143 `addWake` — leftover V92 (yaw Y=±11°, **pas** merger `c62f`).
...
- UnitModels `addWake` `CFrame.Angles` — leftover V92 (yaw Y=±11° fixe, deux sides Wake, recette feel N143 inline, **pas** un rot cuit, **pas** merger `c62f`). Rampes SAM **non** retouchées.
- `CFrame.Angles` de construction restants (`addWake`, `Bow`, `LandingRamp`, `MissileBody`, ailettes) — une fois, pas 60 Hz interpolation. Prochaine après V91 = UnitModels `addWake` (feel N143, yaw Y=±11°).
...
Serveur : invariants + P0 + or plat + `removePlayer` refund + embargo auto + cap 3 transports + passe 16–72 inchangées (passe 73 = client-only).  
...
Client V91 : même check modeles — 4 `Interceptor`, `Y == 5.1 * WORLD_SCALE`. V90 portes `# == 2` **doit rester vert**. V89 toits `# == 2` palier 1 **doit rester vert**. V88 Chimney / StrategicMissile **doit rester vert**. V85 BuildRing Y == 2 **doit rester vert**. Leftover V92 `addWake` **non testé ici**.  
