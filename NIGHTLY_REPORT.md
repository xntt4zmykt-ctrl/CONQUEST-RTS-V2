...
| Claim #193 | Réalité à l’ouverture |
|---|---|
| Effects `floatingText` free-list (N158) | Oui. `textFree` pop O(1). `Parent = nil` + push, pas Destroy. Arbre Billboard + TextLabel + UIStroke. Reset Transparency. Pas de Name. |
...
| Specs N152 / N159 | **N159 corrigé ici.** N152 **laissé ouvert** (pulse Z conservé ; freeze Size=API = visual V74, ne pas merger `c0ec`). |

PRs ouvertes au moment de la revue : #16 P0, hardening jusqu’à #160/`41e2` (N107–N108), feel jusqu’à #193, visuelles #39/…/`aae2` V94 **fermé**. **#193 + cette passe** est le sur-ensemble feel à merger. La ligne P0 sans feel reste distincte. Ne pas merger visual `aae2` / `29b4` / `d645` / `930a` / `6095` / `9fd4` / `f9ec` / `2b04` / `75ce` / `7be5` / `0b3d` / `490f` / `47c0` / `a597` / `54d6` / `6183` / `185a` / `70a5` / `c0ec` / `1b5c` ni hardening `41e2` / `93f6` / `e291` sans rebase.

**Revue autorité :** pas de RemoteFunction ; pas de chemin client gold/troupes/owner ; pas de cycle Server/Shared. `JoinRequest` reste hors IntentValidator (chemin menu dédié, ended+cooldown déjà). N159 est cosmétique client (popup or Overlay). Risques documentés, non corrigés ici (hors N159) : Persistence `math.max` perd les +1 concurrents (N6) ; `RequestSnapshot` buffer owner complet.

**Revue combat/éco :** `areAllied` deux sens + expiry OK ; bots `humanTargetProtected` OK. **Tribus** : `Tribes.decideAttack` n’appelle pas `humanTargetProtected` (88 % skip seulement) — écart feel vs hardening/visual, **non porté** cette passe (gameplay, pas stub). Scan cadran O(carte) encore N9. `Trade.dispatch` `{}` encore (hardening N92, pas sur feel). Aucun bug clair sûr hors N159. `CARRIER_MESH_ID` reste `""` — le chemin mesh n’est pas exercé par le banc (N148 est un crash latent, pas un vert runtime). BuildingModels `animate` n’a plus de `child.Position` 60 Hz. UnitModels n’a plus de `offset.Position` 60 Hz. UnitModels n’a plus de `CFrame.Angles` vivant. `HUD.feedEntries` plafond **sans** `table.remove(1)` (N153). Effects `previewTile` **sans** rewrite CFrame/Color si hover inchangé (N154). Effects `selectTile` **sans** Destroy+recreate (N155). Effects `tileFlash` **sans** Destroy à 0.7 s (N156). Effects `conquestPulse` **sans** Destroy à 1 s (N157). Effects `floatingText` **sans** Destroy à 1.3 s (N158). Overlay `goldPopup` **sans** Destroy à 1.5 s (N159). Serveur feel : `table.remove` bateaux/missiles encore (hardening N105–N106, ne pas porter). UnitModels `place` flamme `Size = Vector3.new` encore (leftover N152, **≠** visual V74 freeze Size=API). Overlay LaunchWake Instance.new+Destroy (leftover N160).

---

## 3. Correctifs livrés (sûrs, server-authoritative)

Feel #19 inchangé. Pas de réinvention : N159 du rapport #193. N152 **non livré** (spec : si le seul patch est un freeze, ne pas livrer).

| Bug | Fichiers | Pourquoi 20K CCU / autorité |
|---|---|---|
| Overlay `goldPopup` Instance.new+Destroy chaque livraison (N159) | `Overlay.luau` (`goldPopup` **seulement**, `onTradeEvent` / LaunchWake / LandingSplash / explosion inchangés), `tests/client.luau` (commentaire livraison + assert reuse dans vagues) | Leftover N158. Free-list `goldFree`, pop O(1). `Parent = nil` + push si encore parenté, pas Destroy. Arbre Billboard Name `GoldPopup` / `Board` conservé. Reset `TextTransparency` / `TextStrokeTransparency` ours vs rival. Tween 1.4 Quad Out/In conservé. Cosmétique. Flame leftover N152 **alors**. HUD N153 **inchangé**. Effects **inchangé**. LaunchWake leftover N160 **alors**. |

**Non modifié (volontaire) :** apply immédiat (N14), câblage `MAX_TILES_PER_TICK` (N11), coalescence skip-si-inchangé (N2 restant), DataStore merge additif (N6), tribus vs capa (N12), fusion Config/ChantierB (N1), cap humains éliminés (N17), heap AimFront vs ChantierB (N18), embargo allié (N19), MAX_BOATS (N25), RequestSnapshot client (N28), landing bonus mort (N33), bateau allié = retraite 25 % (N10.8 design), `stepDoomsday` skip AFK, `seedBeachhead` Attack+queued+Heap (N5 ouvert), pool `building.links`, scan cadran O(carte) (**N9**), corps mort `GameState.stepAttacks` `local collapsing` (**N8**), UnitModels flamme `Size = Vector3.new` (**N152**, **≠** visual V74 freeze — **non livré**), Overlay LaunchWake Instance.new+Destroy (**N160**), flamme Size = API leftover visual V74 fermée Option A — feel **garde** le pulse, ne pas merger, PlacementPreview Size rayon (visual V76, feel Size = API), PlacementPreview early-out hover (visual V77, **pas merger**), Overlay LaunchWake `wakeRot` (visual V78, feel N130 **inline**, **pas merger**), Overlay LandingSplash `wakeRot` (visual V79, feel N131 **inline**, **pas merger**), Overlay DeliveryPulse `wakeRot` (visual V80, feel N132 **inline**, **pas merger**), Overlay Shockwave `wakeRot` (visual V81, feel N133 **inline**, **pas merger**), Effects `conquestPulse` `pulseRot` (visual V82, feel N134 **inline** Y=3 **et** N157 free-list, **pas merger** `490f`), Effects SelectionRing `pulseRot` (visual V83, feel N155 **reuse Part** Name `SelectedTerritory`, **pas merger** `0b3d`), tribus `humanTargetProtected`. Effects / UnitModels / WorldCamera / WorldRenderer / BuildingModels / HUD / serveur **non édités**. Flame **non**. `previewTile` **non**. `selectTile` **non**. `tileFlash` **non**. `conquestPulse` **non**. `floatingText` **non**. Dismiss `table.remove(index)` **non**. LaunchWake **non**. `clearSelection` Destroy **inchangé**. `lossWave` barycentre **inchangé**. `onTradeEvent` **inchangé**.
...
2. **Garder le pulse.** Ne **pas** poser Size = API. Ne **pas** « fermer » trail (N151 déjà). Ne **pas** « fermer » HUD (N153 déjà). Ne **pas** « fermer » previewTile (N154 déjà). Ne **pas** « fermer » selectTile (N155 déjà). Ne **pas** « fermer » tileFlash (N156 déjà). Ne **pas** « fermer » conquestPulse (N157 déjà). Ne **pas** « fermer » floatingText (N158 déjà). Ne **pas** « fermer » goldPopup (N159 déjà). Ne **pas** lire `piece.part.Size` (alloc). Ne pas porter visual. Après N159. Trail **non**. Radar / flag **non**. Wake **non**. `BuildingModels` **non**. Overlay **non**. HUD **non**. Effects **non**. `createMissile` construction **non** (taille initiale `Vector3.new(0.62, 0.62, 2.3)` inchangée).

3. **Si le seul patch possible est un freeze Size=API : ne pas livrer N152. Laisser la ligne inchangée. Livrer N160 seulement.** N152 reste alors ouvert (feel-diverged vs V74). Ne pas inventer un cache Size. Ne pas skip-si-inchangé (N2, pas ici).

4. Tests « navires, missiles et interpolation » leftover N151 trail **et** leftover N148 mesh **et** leftover N147 Fin **doivent rester verts**. Tests « fil de notifications sature » leftover N153 **doivent rester verts**. Tests « calques d'entites, effets et apercu » leftover N155 `selectTile` reuse **doivent rester verts**. Tests « vagues de conquete » leftover N159 `goldPopup` reuse **et** leftover N158 `floatingText` reuse **et** leftover N157 `conquestPulse` reuse **et** leftover N156 `tileFlash` reuse **doivent rester verts**. Tests « livraison : le gain s'affiche sur la gare » leftover N159 **doivent rester verts**. Client **35/35**. `./tests/run.sh`. 6000 ticks serveur inchangé.

5. Fichiers : `UnitModels.luau` (`place` flame **seulement**, **un** `elseif`) **seulement si** un patch feel distinct de V74 existe. Sinon **aucun** fichier flame. `tests/client.luau` **seulement si** le check navires ne mentionne pas encore N152 (commentaire leftover, **garder** N151/N148). `BuildingModels.luau` **non**. `Overlay.luau` **non**. `HUD.luau` **non**. `Effects.luau` **non**. **Ne pas** éditer le serveur. **Ne pas** toucher trail ni HUD ni previewTile ni selectTile ni tileFlash ni conquestPulse ni floatingText ni goldPopup.

**Contraintes :** pas de RemoteFunction. **N152 feel ≠ N151 (trail Z) ≠ visual V74 (freeze Size=API, ne pas merger) ≠ N153 (HUD feed) ≠ N154 (previewTile skip) ≠ N155 (selectTile reuse) ≠ N156 (tileFlash pool) ≠ N157 (conquestPulse pool) ≠ N158 (floatingText pool) ≠ N159 (goldPopup pool) ≠ N160 (LaunchWake pool) ≠ N2 (skip-si-inchangé).** Non réentrant. Ne pas fusionner avec N160 dans le même worker. Pulse Z **conservé** — ne pas figer à 1.8. Un `elseif` flame.

---

### ISSUE-N160 — Overlay LaunchWake Instance.new+Destroy chaque insert navire (feel)

**Priorité :** P3 alloc client Overlay. Leftover explicite après N159 (`goldPopup` free-list déjà). Distinct de N152 (UnitModels Size), de N153 (HUD feed), de N154 (previewTile skip), de N155 (`selectTile` reuse), de N156 (`tileFlash` pool), de N157 (`conquestPulse` pool), de N158 (`floatingText` pool), de N159 (`goldPopup` pool), de N130 (LaunchWake euler **déjà** inline), de visual V78 (`wakeRot` cuit — **ne pas merger** `6183`). `trackUnit` insert navire (`not isMissile`) **seulement**. Ne pas toucher `goldPopup`. Ne pas toucher LandingSplash. Ne pas toucher `routePart` (partagé Splash/Pulse/Shockwave/Smoke). Ne pas toucher HUD. Ne pas toucher UnitModels. Ne pas toucher Effects.

**Problème :** N159 ferme le pool popups or. N152 reste ouvert (freeze interdit). Reste, **chaque premier insert d’un navire** (`trackUnit`, `not isMissile`, `unit` absent) :
...
TweenService:Create(wake, TweenInfo.new(0.7, ... Out), { Size = Vector3.new(0.16, 9, 9), Transparency = 1 }):Play()
...
Instance.new (via `routePart`) + Destroy à 0.8 s par spawn navire. `goldPopup` **réutilise déjà** (`goldFree`, N159). `conquestPulse` **réutilise déjà** (`pulseFree`, N157). LaunchWake non. Distinct de leftover N152 (`Size = Vector3.new` flamme). Distinct de leftover N159 (`goldPopup` pool **déjà** — **ne pas** partager `goldFree`). Distinct de leftover N130 (euler anneau **déjà** inline). Distinct de leftover N131 (LandingSplash Destroy — leftover suivant, **ne pas** fusionner). Distinct de visual V78 (`wakeRot` cuit — **ne pas merger** `6183`). Name `LaunchWake` **inchangé**. Shape Cylinder **inchangé**. Size départ `Vector3.new(0.16, 2.5, 2.5)` **inchangé**. Color `174, 231, 244` **inchangé**. Material Neon **inchangé**. Transparency départ `0.34` **inchangée**. Offset Y `OCEAN_LEVEL + 0.12` **inchangé**. Euler `fromEulerAnglesYXZ(0, 0, math.rad(90))` **inchangé** (N130 inline). Tween 0.7 Quad Out Size `(0.16, 9, 9)` / Transparency 1 **inchangé**. `task.delay` 0.8 s **inchangé**. Garde `if wake.Parent` **inchangée** (devient `Parent = nil` + push). Garde `not isMissile` **inchangée**. `routePart` **non modifié** (LandingSplash / DeliveryPulse / Shockwave / BlastSmoke s’en servent encore).

**Pourquoi 20K CCU :** leftover N159. 8 clients × (transports + cargos + carriers) spawns × `Instance.new` + Tween + Destroy 0.8 s. Pas d’autorité (cosmétique Overlay). Popup or **déjà** N159 — ne pas y revenir. Pulse Effects **déjà** N157 — ne pas y revenir. Flame leftover N152 **alors** (ne pas freeze). Visual V78 `wakeRot` **interdit** (feel ≠ visual, euler déjà N130 inline).

**Worker :**

1. Dans `trackUnit` insert navire (`not isMissile`) seulement : free-list d’ancres (`wakeFree` tableau, pop O(1)). Si une Part libre : **réutiliser** (pas `routePart`). Writes : `Size = Vector3.new(0.16, 2.5, 2.5)` (le tween l’avait mise à `(0.16, 9, 9)`), `Transparency = 0.34`, `CFrame` euler N130 inline (`OCEAN_LEVEL + 0.12`), `Parent = self.root`. Name `LaunchWake` **conservé**. **Ne pas** Destroy à 0.8 s : `Parent = nil` + push free-list (`if wake.Parent` devient push seulement si encore parenté — un second delay fantôme ne double-push pas). Sinon : création inchangée via `routePart` (Name `LaunchWake`, Size `0.16, 2.5, 2.5`, Cylinder, Neon). Tween Create **conservé** à chaque appel (Size + Transparency restart). Pas de `self.live` (c’est Effects `tileFlash`). **Ne pas** wrapper un record. **Ne pas** partager `goldFree` / `textFree` / `pulseFree` / `flashFree`. **Ne pas** modifier `routePart`.

2. **Garder l’insert.** Ne **pas** recréer au reuse. Ne **pas** porter visual V78 (`wakeRot` cuit). Ne **pas** retirer Name `LaunchWake`. Ne **pas** changer Y `OCEAN_LEVEL + 0.12`. Ne **pas** cuire `wakeRot`. Ne **pas** toucher `goldPopup` / N159. Ne **pas** toucher LandingSplash / N131. Ne **pas** toucher DeliveryPulse / N132. Ne **pas** toucher Shockwave / N133. Ne **pas** geler Size (pas N152). Après N159. Flame **non** (N152). HUD **non**. `UnitModels` **non**. Effects **non**. `applyUnits` despawn **non**. PlacementPreview **non**. `stepInterpolation` **non**.

3. Tests « navires, missiles et interpolation » leftover N130 euler **et** leftover N151 trail **et** leftover N148 mesh **doivent rester verts** (insert 3 navires + 1 missile, pas d’erreur LaunchWake). Tests « livraison : le gain s'affiche sur la gare » leftover N159 **doivent rester verts** (Name `GoldPopup` sans flush). Tests « vagues de conquete » leftover N159 `goldPopup` reuse **et** leftover N158 `floatingText` reuse **doivent rester verts**. Check « apercu de placement » leftover N129 **doit rester vert**. Client **35/35**. `./tests/run.sh`. 6000 ticks serveur inchangé.

4. Test : banc client `navires` **doit rester vert**. Après flush des delays de wake **dans le check vagues** (déjà un `testFlushDelays` — les LaunchWake du check navires y tombent), `wakeFree` non vide ; un second `applyUnits` avec un **nouvel** `id` navire réparente la même Part (`rawequal`, Name `LaunchWake`). **Réécrire** Size `(0.16, 2.5, 2.5)` et Transparency `0.34` au reuse sinon l’anneau est invisible (tween l’avait mis à 1) ou trop grand. Ne **pas** flush dans le check « navires » (les wakes restent parentés comme les GoldPopup de « livraison »). Check vagues leftover N159. Check fil leftover N153. Check navires leftover N151 `offset.Z` / leftover N152 flame. Client **35/35**. `./tests/run.sh`. 6000 ticks serveur inchangé.

5. Fichiers : `Overlay.luau` (`trackUnit` insert navire LaunchWake **seulement**). `tests/client.luau` **seulement si** le check navires ne mentionne pas encore N160 (commentaire leftover, **garder** N130 euler / N151 trail). `Effects.luau` **non**. `HUD.luau` **non**. `UnitModels.luau` **non**. `PlacementPreview.luau` **non**. **Ne pas** éditer le serveur. **Ne pas** toucher flame ni HUD ni `previewTile` ni `selectTile` ni `tileFlash` ni `conquestPulse` ni `floatingText` ni `goldPopup`. **Ne pas** toucher LandingSplash / DeliveryPulse / Shockwave. **Ne pas** modifier `routePart`.

**Contraintes :** pas de RemoteFunction. **N160 feel ≠ N159 (goldPopup pool) ≠ N158 (floatingText pool) ≠ N157 (conquestPulse pool) ≠ N156 (tileFlash pool) ≠ N155 (selectTile reuse) ≠ N154 (previewTile skip) ≠ N152 (flame Size, ne pas freeze V74) ≠ N153 (HUD feed) ≠ N130 (LaunchWake euler déjà) ≠ visual V78 (LaunchWake wakeRot, ne pas merger `6183`) ≠ N131 (LandingSplash Destroy, leftover suivant) ≠ N2 (skip-si-inchangé replication).** Non réentrant. Ne pas fusionner avec N152 dans le même worker. Free-list — ne pas skip CFrame si l’origine change. Un insert `trackUnit` — ne pas splitter `applyUnits`. `task.delay` 0.8 s **conservé**. Ne pas `table.remove` la free-list (pop O(1) depuis la fin). Ancre `Parent = nil` **pas** Destroy. Distinct `goldFree` / `textFree` / `pulseFree` / `flashFree` — **ne pas** partager les free-lists. **Reset Size + Transparency** au reuse — oubli = anneau fantôme. Name `LaunchWake` **obligatoire**. Euler N130 **inline** — ne pas cuire `wakeRot`. Ne pas modifier `routePart`. Garde `not isMissile` **obligatoire** (les missiles n’ont pas de wake).
...
| N2 | Delta `stats` + UnitSnapshot dirty | P1 | ouvert (`buildPrices` → **N75 fait** ; … ; trail → **N151 fait** ; HUD feed → **N153 fait** ; previewTile → **N154 fait** ; selectTile → **N155 fait** ; tileFlash → **N156 fait** ; conquestPulse → **N157 fait** ; floatingText → **N158 fait** ; goldPopup → **N159 fait** ; reste skip-si-inchangé) |
...
| N10 | Divers P3 | P3 | ouvert (`Buildings.contextFor` → **N85 fait** ; … ; HUD feed → **N153 fait** ; previewTile skip → **N154 fait** ; selectTile reuse → **N155** ; tileFlash pool → **N156** ; conquestPulse pool → **N157** ; floatingText pool → **N158** ; goldPopup pool → **N159** ; Overlay LaunchWake pool = **N160**) |
...
| N34–N151, N153–N159 | (voir rapport #193) | — | **faits** |
| N152 | UnitModels `place` flamme `Size = Vector3.new` 60 Hz | P3 | **ouvert** (`place`, pulse Z **conservé**, **≠** visual V74 freeze ; **non livré** passes 61–68) |
| N160 | Overlay LaunchWake Instance.new+Destroy | P3 | **nouveau** (`trackUnit` insert navire free-list, Name `LaunchWake` **conservé**, euler N130 **inline**, **≠** N159 `goldFree` / visual V78 `wakeRot`) |

Textes worker-ready N1–N25, N28, N33 : PR #21 / #22 / #24 / #26 / #29 / #32 / #34 / #36 / #38 / #41 / #42 / #45 / #48 / #51 / #53 / #56 / #59 / #62 / #65 / #68 / #71 / #75 / #78 / #82 / #86 / #89 / #93 / #96 / #99 / #101 / #106 / #108 / #111 / #114 / #118 / #121 / #125 / #128 / #131 / #133 / #136 / #140 / #144 / #147 / #150 / #153 / #155 / #158 / #161 / #163 / #165 / #167 / #169 / #171 / #173 / #176 / #178 / #181 / #183 / #185 / #187 / #189 / #191 / #193 `NIGHTLY_REPORT.md` historique.
...
| `TILE_SIZE` | 12 | n/a | oui (N101 lerp monde … N159 goldPopup pool) |
