# Nightly report — passe 74 (revue PR #186)

**Branche revue :** `cursor/analyse-nocturne-du-codebase-930a` (PR #186, `dc4d71f`)  
**Branche de correctifs :** `cursor/analyse-nocturne-du-codebase-d645`  
**Date :** 2026-08-26  
**Banc :** `./tests/run.sh` — serveur **vert**, client **34/34 vert**. `error()` si un invariant casse (Luau CLI sans `os.exit`).

Revue de PR #186 (`BuildingModels` `buildSam` Interceptor `fromEulerAnglesYXZ` — HEAD visuel, V91). Correctifs sûrs, sans merger feel `c62f`/`4885`/`a8e1`/`c299`/`595e`/`bfcc` ni hardening `41e2`/`93f6`.
...
- **PR #186 :** V91 intact (`fromEulerAnglesYXZ` pitch X=-31° fixe, Interceptor `buildSam`, pas de `rampRot`). Sillages V92 **fermées ici**. Portes V90 / toits V89 / `cylinder()` V88 / Rock V87 / TreeTrunk V86 / BuildRing V85 / OceanGlint V84 / SelectionRing V83 / ConquestPulse V82 / Shockwave V81 / DeliveryPulse V80 / LandingSplash V79 / LaunchWake V78 / early-out V77 / Size V76 / rot V75 **non retouchés**. Rien à revert.

---

## Specs worker (reste)

Ne pas merger feel `c62f`/`4885`/`a8e1`/`c299`/`595e`/`bfcc`/`95ed` ni hardening `41e2`/`93f6` sur cette branche sans rebase. Porter **une** recette à la fois. Feel N129 (footprint hauteur **0.4**, **pas** de pulse) ≠ visual V75/V76/V77 — ne pas merger. Feel N134 `Y = 3` **non** porté (visuel = surface + 0.8). Feel N135 OceanGlint **fermée** (V84) — **ne pas merger** `595e`. Feel N136 BuildRing **fermée** (V85, PR #175) — **ne pas merger** `595e`. Feel N137 TreeTrunk **fermée** (V86, PR #177) — **ne pas merger** `c299`. Feel N138 Rock **fermée** (V87, PR #179) — **ne pas merger** `c299`. Feel N139 `cylinder()` **fermée** (V88, PR #180) — **ne pas merger** `a8e1`. Feel N140 toits usine **fermée** (V89, PR #182) — **ne pas merger** `a8e1`. Feel N141 portes silo **fermée** (V90, PR #184) — **ne pas merger** `4885`. Feel N142 rampes SAM **fermée** (V91, PR #186) — **ne pas merger** `4885`. Feel N143 `addWake` **fermée** ici (V92 inline) — **ne pas merger** `c62f`. Feel N144 `Bow` — **ne pas merger** `c62f`.
...
### ISSUE-V93 — UnitModels `Bow` `CFrame.Angles`

**Problème.** V92 ferme `CFrame.Angles` sur les **deux sillages** de `addWake` (yaw Y=±11° fixe). Restent les **deux proues** navire dans `UnitModels` :

```
CFrame.new(0, -0.2, -10.8) * CFrame.Angles(0, math.pi, 0)   -- carrier, buildCarrier
CFrame.new(0, 0.05, -5.9) * CFrame.Angles(0, math.pi, 0)    -- transport/cargo, createBoat
```

Yaw Y **fixe** π (deux sites, translations distinctes). `Name = Bow`. `WedgePart`. Distinct de V92 (sillages, yaw Y=±11°, `addWake` — ne **pas** retoucher les Wake). Distinct de leftover V94 (`LandingRamp` pitch X=-8°, kind==1 seulement). Distinct de V78 (LaunchWake Overlay — **autre objet**). Construction de modèle, **pas** 60 Hz interpolation. `addPart` pose `part.CFrame = offset` ; Overlay `place` compose `frame * offset` ensuite. Size carrier `5.6, 1.6, 3.6` / transport `4.8, 1.65, 3.3` / NAVY / Metal **inchangés**. Call sites `buildCarrier` **et** `createBoat` (kind==1/2, avant pont) **non édités** hors la ligne CFrame.

**20K CCU.** 8 clients × pose navire (spawn + capture de côte). Pas d’autorité. Même classe que feel N144 (Angles → `fromEulerAnglesYXZ` inline, yaw Y=π — **ne pas** cuire un rot, translations Y/`Z` distinctes, comme V86 `trunkRot` / V88 `cylinderRot` / V89 `roofRot` / V90 `doorRot` / V91 `rampRot` / V92 `wakeRot` interdits).

**Faire.** Remplacer `CFrame.Angles(0, math.pi, 0)` par `CFrame.fromEulerAnglesYXZ(0, math.pi, 0)` aux **deux** sites. Hot path carrier : `CFrame.new(0, -0.2, -10.8) * CFrame.fromEulerAnglesYXZ(0, math.pi, 0)`. Hot path transport/cargo : `CFrame.new(0, 0.05, -5.9) * CFrame.fromEulerAnglesYXZ(0, math.pi, 0)`. Position / Size / Color / Material / Name **inchangés**. Ne **pas** cuire un `bowRot` (translations Y/`Z` distinctes, recette feel N144 **inline**). Ne **pas** retoucher `addWake` (V92 déjà). Ne **pas** éditer `LandingRamp` (leftover V94). Ne **pas** éditer `MissileBody` / `Fin`. Ne **pas** changer `UnitModels.place` (V73 déjà). Ne **pas** changer Overlay (V71/V72/V78 déjà). Ne **pas** merger feel `c62f`.

**Contraintes.** Client-only. **V93 visual ≠ V92 (Wake yaw Y=±11°) ≠ V91 (Interceptor pitch X=-31°) ≠ V94 leftover (`LandingRamp` X=-8°).** Recette feel N144 — **ne pas merger** `c62f`. Client 34/34 (check navires V92 Wake `# == 2` Y == -0.68 **doit rester vert**. Check V91 Interceptor `# == 4` Y == 5.1 × scale **doit rester vert**. Check V90 portes `# == 2` **doit rester vert**. Check V78 LaunchWake **doit rester vert**). **Ne pas** éditer le serveur. **Ne pas** éditer `BuildingModels.luau`. **Ne pas** éditer `WorldRenderer.luau`. **Ne pas** éditer `Effects.luau`. **Ne pas** éditer `Overlay.luau`. **Ne pas** éditer `addWake` / `LandingRamp` / `MissileBody`.

**Tester.** Check « navires, missiles et interpolation » : après spawn 3 navires, chaque modèle navire a 1 Part `Name == Bow`. Offset local carrier `Y == -0.2` ; transport/cargo `Y == 0.05` (construction ; Overlay `place` compose ensuite). Missile skip. V92 Wake `# == 2` Y == -0.68 **doit rester vert**. V91 Interceptor `# == 4` **doit rester vert**. V78 LaunchWake Y = OCEAN+0.12 **doit rester vert**. `./tests/run.sh`. Client 34/34.

**Fichiers.** `UnitModels.luau` (`Bow` **seulement** les deux lignes CFrame). `tests/client.luau` check navires + commentaire leftover. `addWake` **non**. `BuildingModels.luau` **non**. `WorldRenderer.luau` **non**. `Effects.luau` **non**. `Overlay.luau` **non**. `init.client.luau` **non**.

---

## Hors scope volontaire

- Merger feel `c62f`/`4885`/`a8e1`/`c299`/`595e`/`bfcc`/`95ed` / hardening `41e2`/`93f6` sur #186.
...
- Feel N143 `addWake` — **porté ici** (V92 inline, yaw Y=±11° fixe, **pas** merger `c62f`).
- Feel N144 `Bow` — leftover V93 (yaw Y=π, **pas** merger `c62f`).
...
- UnitModels `Bow` `CFrame.Angles` — leftover V93 (yaw Y=π fixe, deux sites carrier + transport/cargo, recette feel N144 inline, **pas** un rot cuit, **pas** merger `c62f`). Sillages **non** retouchés.
- `CFrame.Angles` de construction restants (`Bow`, `LandingRamp`, `MissileBody`, ailettes) — une fois, pas 60 Hz interpolation. Prochaine après V92 = UnitModels `Bow` (feel N144, yaw Y=π).
...
Serveur : invariants + P0 + or plat + `removePlayer` refund + embargo auto + cap 3 transports + passe 16–73 inchangées (passe 74 = client-only).  
