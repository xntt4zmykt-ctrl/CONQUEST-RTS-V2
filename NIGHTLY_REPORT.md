# Nightly report — passe 62 (revue PR #164)

**Branche revue :** `cursor/analyse-nocturne-du-codebase-54d6` (PR #164, `add6dc7`)  
**Branche de correctifs :** `cursor/analyse-nocturne-du-codebase-a597`  
**Date :** 2026-08-26  
**Banc :** `./tests/run.sh` — serveur **vert**, client **34/34 vert**. `error()` si un invariant casse (Luau CLI sans `os.exit`).

Revue de PR #164 (`Overlay` LandingSplash `wakeRot` — HEAD visuel, V79). Correctifs sûrs, sans merger feel `5aa9`/`bfcc` ni hardening `41e2`/`93f6`.
...
- **PR #164 :** LandingSplash V79 intact (`wakeRot` réutilisé, 3 navires → 3 splash, Y + 0.14, missile skip, relot `rawequal`). LaunchWake V78 / early-out V77 / Size V76 / rot V75 **non retouchés**. Rien à revert.

---

## Specs worker (reste)

Ne pas merger feel `5aa9`/`bfcc` ni hardening `41e2`/`93f6` sur cette branche sans rebase. Porter **une** recette à la fois. Feel N129 (footprint hauteur **0.4**, **pas** de pulse) ≠ visual V75/V76/V77 — ne pas merger.
...
### ISSUE-V81 — Overlay Shockwave `CFrame.Angles` à l’explosion

**Problème.** V80 ferme `CFrame.Angles` à **l’arrivée camion** (`DeliveryPulse`). Reste, à **chaque explosion** (`Overlay.explosion`) :
...
Rotation constante (cylindre plat) — même constante que V75 `footprintRot` / V78–V80 `wakeRot`. Feel N133 a déjà cuit `fromEulerAnglesYXZ` sur feel `bfcc` : **porter la recette, ne pas merger**. Distinct de V80 (arrivée camion, `route.to`). Distinct de V79 (despawn navire, Y + 0.14). Distinct de V78 (spawn navire, Y + 0.12). Distinct de Effects `conquestPulse` (leftover V82 / feel N134, `Y` surface + 0.8 visuel, pas le `Y = 3` feel). Distinct de sphère Blast / fumée (pas de euler). Événement explosion, pas 60 Hz interpolation. Check navires a déjà `overlay:explosion(50, 50, 9)` — étendre, ne pas dupliquer le check.

**20K CCU.** 8 clients × frappe. Pas d’autorité. Même classe que V78–V80, prochaine constante cylindre à cuire.

**Faire.** Réutiliser `self.wakeRot` (constante identique — **ne pas** inventer `shockRot`). Hot path : `CFrame.new(ground.X, Config.TERRAIN_HEIGHT[Config.TERRAIN.PLAINS] + 0.5, ground.Z) * self.wakeRot`. Y = PLAINS + 0.5 **inchangé**. Tween Size/Transparency **inchangé**. Sphère Blast / fumée **inchangées**. Ne **pas** porter Effects `conquestPulse` dans le même commit. Ne **pas** changer DeliveryPulse (V80 déjà). Ne **pas** changer le lerp camion 60 Hz (V57/V64/V72 déjà).

**Contraintes.** Client-only. **V81 visual ≠ V80 (DeliveryPulse) ≠ V79 (LandingSplash) ≠ V78 (LaunchWake).** Recette feel N133 — **ne pas merger** `bfcc`. Client 34/34 (check pose/capture V80 **doit rester vert**. Check navires V79 LandingSplash **doit rester vert**). **Ne pas** éditer le serveur. **Ne pas** éditer `Effects.luau`. **Ne pas** éditer `PlacementPreview.luau`. **Ne pas** éditer `UnitModels.luau`.

**Tester.** Check navires : après `overlay:explosion(50, 50, 9)`, Shockwave présent, `math.abs(shock.CFrame.Y - (PLAINS + 0.5)) < 1e-6`, `rawequal(overlay.wakeRot, bakedWake)`. Check pose/capture V80 **doit rester vert**. `./tests/run.sh`. Client 34/34.

**Fichiers.** `Overlay.luau` (`explosion` Shockwave **seulement**). `tests/client.luau` check navires + commentaire leftover. `Effects.luau` **non**. `PlacementPreview.luau` **non**. `UnitModels.luau` **non**. `init.client.luau` **non**.

---

## Hors scope volontaire

- Merger feel `5aa9`/`bfcc` / hardening `41e2`/`93f6` sur #164.
...
- Overlay Shockwave `CFrame.Angles` — leftover V81 (feel N133, `explosion`, `PLAINS + 0.5`).
...
- Pulse explosion `CFrame.Angles` — événement (V81).
...
Serveur : invariants + P0 + or plat + `removePlayer` refund + embargo auto + cap 3 transports + passe 16–61 inchangées (passe 62 = client-only).  
